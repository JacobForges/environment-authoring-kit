...
- **Grid snap:** `SurfaceFullWorldGridAnchor` + `SnapFullWorldTerrainGridNow()`; layout audit uses anchor-aware `ExpectedOrigin` and tight seam adjacency (fixes false ~66 m gaps).
- **Faster flat build:** skip per-tile seam queue by default; horizon batch connectivity-only; outer-ring batch locks + 120 s budget.
- **Open world:** `SurfaceOpenWorldGridExpansion` — target ~289 tiles (Chebyshev 8, 17×17), manifest `OpenWorldGridManifest.json`, one ring per menu run.
- **Menus renamed:** Snap Surface Terrain Grid (81 Tiles); Expand Open World — Add Next Terrain Ring.
- **Docs:** [SURFACE_TERRAIN_GRID_AND_OPEN_WORLD.md](../Packages/com.cursor.environment-authoring-kit/docs/SURFACE_TERRAIN_GRID_AND_OPEN_WORLD.md), updated READMEs and [FIRST_RUN_AFTER_SESSION.md](../Packages/com.cursor.environment-authoring-kit/docs/FIRST_RUN_AFTER_SESSION.md).

---

## 2026-05-25

### FullWorld 81-tile grid + Hub-first monitoring (docs + pipeline)

- **Terrain:** 9×9 FullWorld grid (**81** terrains) — added **32** `SurfaceMountainHorizonTile_*` (Chebyshev 4); play 9 + foothill 16 + peak 24 unchanged in role.
- **Hub:** primary live monitor during builds; Pipeline Console **optional pop-out** only (no auto-open; saves RAM).
- **Sequential FullWorld terrain** (default): finish each tile’s sculpt + seams before the next; `QueueWhenIdle` between terraform steps.
- **Docs:** new [FULLWORLD_TERRAIN_AND_HUB.md](../Packages/com.cursor.environment-authoring-kit/docs/FULLWORLD_TERRAIN_AND_HUB.md); root + package READMEs and major package docs updated for **122** queued steps and Hub workflow.
