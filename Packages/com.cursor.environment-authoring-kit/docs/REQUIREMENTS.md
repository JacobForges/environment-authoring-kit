...
| S-02 | Support **fixed 9-tile 3×3 square** via `SurfaceTerrainTileExpansion.NineTileRingOffsets` (sequential build, one tile/frame). | Must |
...
| Grass | 320 | 210 |
| Bushes | 195 | 138 |
| Ground cover | 228 | 158 |

**Placement rules:**

- **Unified full-map spread** (one pass per category): **2×** base grid spacing on **every locked terrain tile**, plus interstitial half-cell offset in the same pass (no separate wide-spread pipeline pass).
- Slots generated inside **each tile’s footprint** (not only a radial annulus around play center).
- Placement fills **minimum per tile** before global fill (`OrderSlotsForFullAreaSpread`).
- Optional **polish** top-up toward contract after all categories.
...
| Q-06 | **`WorldLayoutAudit.json`** after surface + at cave finalize (seams, floaters, overlap, mouth). | Must |
| Q-07 | FullWorld blocks cave queue when `blocksCaveQueue` unless `CAVE_LAYOUT_PLAN_FORCE=1`. | Must |
...
| N-05 | Editor queue depth limits; per-label coalescing (`CaveBuildEditorQueueSafeguard`); step watchdog for stuck pipeline. Full AAA Rebuild forces per-phase prompts + non-additive surface — see `docs/FULL_AAA_REBUILD.md`. |
...
4. Hub or Generated status shows **cave geo 1/13 … 13/13** on fresh or invalidated ladder builds.  
