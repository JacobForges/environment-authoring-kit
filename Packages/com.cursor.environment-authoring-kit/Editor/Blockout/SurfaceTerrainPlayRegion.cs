...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    static class SurfaceTerrainPlayRegion
    {
        ...
        public static bool TryTerrainAtWorldXZ(Terrain mainTerrain, float worldX, float worldZ, out Terrain terrain)
        {
            ...
            {
                if (wild != null && ContainsWorldXZ(wild, worldX, worldZ))
                {
                    terrain = wild;
                    return true;
                }
        ...
        /// <summary>Ground anchor XZ/Y when present; otherwise main terrain tile center.</summary>
        public static Vector3 ResolveUnifiedPlayCenter(SceneGroundInfo ground, Terrain mainTerrain)
        {
            if (ground != null && ground.HasAnchor)
                return new Vector3(ground.AnchorWorld.x, ground.SurfaceY, ground.AnchorWorld.z);
        ...
        /// <summary>All Environment Kit surface tiles (play + wilderness + open world) for props / AAA landmarks.</summary>
        public static List<Terrain> CollectAllKitGroundTerrains(Terrain mainTerrain)
        {
            var set = new HashSet<Terrain>();
            if (mainTerrain?.terrainData != null)
                set.Add(mainTerrain);

            foreach (var t in CollectSurfaceTerrains(mainTerrain))
            {
                if (t != null)
                    set.Add(t);
            }
            ...
            {
                if (terrain?.terrainData == null || !terrain.CompareTag("Ground"))
                    continue;

                if (!IsKitSurfaceTerrainName(terrain.name))
                    continue;
        ...
        static bool IsKitSurfaceTerrainName(string name) =>
            !string.IsNullOrEmpty(name) &&
            (name.StartsWith("SurfaceTerrain", StringComparison.Ordinal) ||
             name.StartsWith(SurfaceTerrainTileExpansion.MountainFoothillTileNamePrefix, StringComparison.Ordinal) ||
             ...
             name.StartsWith(SurfaceTerrainTileExpansion.OpenWorldTileNamePrefix, StringComparison.Ordinal));
