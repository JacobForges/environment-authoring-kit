...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    public static class TerrainBuildRungPromptExporter
    {
        ...
        public static void WriteTailoredFixPrompt(
            ...
            int seed,
            int fixRound) =>
            WriteTailoredFixPrompt(rung, report, seed, fixRound, "terrain", "terrain_meat_loop");
            ...
            string phaseId)
        {
            ...
            workflowEnv = string.IsNullOrEmpty(workflowEnv) ? "terrain" : workflowEnv;
            phaseId = string.IsNullOrEmpty(phaseId) ? "terrain_meat_loop" : phaseId;

            CaveBuildPhasePromptBridge.ExportResearchAgentPrompt(phaseId, rung, out _);
            ...
            var meatLabel = workflowEnv switch
            {
                "terrain_geo" => "Geo meat loop (heightfield / tiles / trails — no default land)",
                "terrain_props" => "Props meat loop (trees / grass / bushes / ground cover)",
                _ => "Terrain meat loop",
            };
            sb.AppendLine($"# {meatLabel} — fix pass");
            sb.AppendLine();
            sb.AppendLine($"**Workflow:** `{workflowEnv}` | **Phase:** `{phaseId}`");
            sb.AppendLine($"**Grading task:** `{report.GradingTask}` | **Mode:** `{report.GradingMode}`");
            sb.AppendLine($"**Fix round:** {fixRound + 1} | **Seed:** {seed} | **Rung:** `{rung}`");
            sb.AppendLine(
                $"**Overall:** {report.OverallScore}/100 ({report.LetterGrade}) — stage pass {SurfaceTerrainBuildLadder.StagePassScore}+");
            sb.AppendLine($"- `{hub}/{SurfaceDefaultTerrainDetector.StatusRel}`");
            sb.AppendLine();
