...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildResearchCacheBridge
    {
        ...
        public static bool ShouldSkipBriefTsxSyncForScope(string scope)
        {
            if (ForceNetworkResearchSync())
                return false;
            if (!ShouldUseOnDiskResearchFastPath())
            ...
            var hub = CaveBuildCursorSettings.ResolveHubRoot();
            var rel = scope switch
            {
                "terrain" => TerrainExecutionBriefPath,
                "cave" => CaveExecutionBriefPath,
                _ => ExecutionBriefPath,
            };
            return File.Exists(Path.Combine(hub, rel));
        }

        static bool ShouldDeferBriefTsxSync() =>
            ...
            out string message)
        {
            ...
            if (ShouldDeferBriefTsxSync() || ShouldSkipBriefTsxSyncForScope(scope))
            {
                message = ShouldSkipBriefTsxSyncForScope(scope)
                    ? $"Using on-disk {scope} execution brief (skipped tsx during active build)."
                    : $"Deferred {scope} execution brief export until build idle (non-blocking only).";
                return true;
            }
