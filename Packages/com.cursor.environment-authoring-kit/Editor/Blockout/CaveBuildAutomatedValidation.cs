...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildAutomatedValidation
    {
            ...
            WorldGenerationRequest request)
        {
            ...
            RunStepBody(step, caveRoot, ground, request);
        }

        static void RunStepBody(
            int step,
            Transform caveRoot,
            SceneGroundInfo ground,
            WorldGenerationRequest request)
        ...
        public static void RunFinalProbe(Transform caveRoot)
        {
            ...
            CaveBuildActionPacing.ScheduleLight(
                () =>
                {
                    var report = CaveRouteProbeRunner.RunPaced(caveRoot);
                    ExportFinalProbe(caveRoot, report);
                },
                CaveBuildPipelineDomains.QueueLabel("cave validate — final route probe"));
        }

        static void ExportFinalProbe(Transform caveRoot, CaveRouteProbeRunner.ProbeReport report)
        {
        ...
        static void RunCombatCheck(Transform caveRoot, WorldGenerationRequest request)
        {
            ...
            RunCombatProbeBatch(caveRoot, request, firstPass: true);
        }

        static void RunCombatProbeBatch(Transform caveRoot, WorldGenerationRequest request, bool firstPass)
        {
            CaveBuildActionPacing.TouchQueueActivity();
            ...
            RunCombatProbeBatch(caveRoot, request, firstPass: false);
        }
    }
}
#endif
