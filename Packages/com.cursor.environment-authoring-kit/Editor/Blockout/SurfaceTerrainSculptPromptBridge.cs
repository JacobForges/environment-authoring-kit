...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceTerrainSculptPromptBridge
    {
            ...
            System.Action onComplete = null)
        {
            if (CaveBuildEditorResponsiveness.IsLongBuildActive)
            {
                CaveBuildActionPacing.ScheduleLight(
                    () =>
                    {
                        ExportBeforeSculptSync(request, ground, seed, playCenter, extentMeters);
                        onComplete?.Invoke();
                    },
                    CaveBuildPipelineDomains.QueueLabel("terrain sculpt prompt export"));
                return;
            }
        ...
        static void ExportBeforeSculptSync(
            WorldGenerationRequest request,
            SceneGroundInfo ground,
            int seed,
            Vector3 playCenter,
