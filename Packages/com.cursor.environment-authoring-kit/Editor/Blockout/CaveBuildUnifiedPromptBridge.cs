...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildUnifiedPromptBridge
    {
            ...
            out bool awaitingPromptTsx)
        {
            ...
            {
                terrainBrief = CaveBuildResearchCacheBridge.ShouldSkipBriefTsxSyncForScope("terrain")
                    ? "on-disk (skipped tsx during build)"
                    : "deferred until build idle";
                caveBrief = CaveBuildResearchCacheBridge.ShouldSkipBriefTsxSyncForScope("cave")
                    ? "on-disk (skipped tsx during build)"
                    : "deferred until build idle";
            }
            else
            {
                CaveBuildResearchCacheBridge.SyncTerrainResearchExecutionBrief(phaseId, meatPass, out terrainBrief);
                CaveBuildResearchCacheBridge.SyncCaveResearchExecutionBrief(phaseId, meatPass, out caveBrief);
            }

            if (CaveBuildPhasePromptBridge.RequiresNonBlockingTsx)
            {
                if (!CaveBuildAaaSessionPolicy.ForcePerPhasePromptExport &&
                    CaveBuildPromptExportSession.HasValidateResearchPromptArtifactsOnDisk())
                {
                    message =
                        $"Prompts on disk for `{phaseId}` — using cached research prompts " +
                        $"(terrain brief: {terrainBrief}; cave brief: {caveBrief}).";
                    LogDebouncedResearchRefresh(phaseId, message);
                ...
                {
                    message =
                        $"Prompts on disk for `{phaseId}` (per-script skip; using cached research prompts).";
                    CaveBuildPromptExportSession.MarkFresh(seed);
                    return true;
                }
                ...
                message =
                    $"Prompt export deferred (async unavailable): {message}. Brief export deferred during build.";
                return true;
            }
            ...
            switch (step)
            {
                case AsyncRefreshStep.Unified:
                    started = CaveBuildPhasePromptBridge.TryBeginRunTsx(
                        ...
                        CaveBuildAaaSessionPolicy.ForcePerPhasePromptExport ? "CAVE_FORCE_PROMPT_EXPORT=1" : null,
        ...
        static void LogDebouncedResearchRefresh(string phaseId, string message)
        {
            LogDebouncedWarning($"research-refresh:{phaseId}", "[CaveBuild] " + message);
        }
        ...
        {
            if (phaseId != null && phaseId.StartsWith("mountain_"))
