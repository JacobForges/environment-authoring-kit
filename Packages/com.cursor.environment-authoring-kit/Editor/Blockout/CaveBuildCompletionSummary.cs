...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildCompletionSummary
    {
        ...
        {
            cancelled = true;
            var hubOpen = EnvironmentKitHubWindow.IsOpen;
            if (surfaceScope == SurfaceBuildScope.FullWorld)
            {
                var resolved = SceneGroundResolver.EnforceFullWorldGround(ground);
                SceneGroundResolver.ApplyGround(ground, resolved);
            }

            CaveBuildDialogPolicy.BeginUnifiedSession();
            Debug.Log(
                $"[CaveBuild] Start confirmed automatically. Scope={surfaceScope}, " +
                $"Ground={(ground.HasAnchor ? ground.Anchor.name : "(none)")}, Portal={portalName}, " +
                $"Hub={(hubOpen ? "open" : "closed")}.");
            cancelled = false;
            return true;
        }
            ...
            bool showDialog = true)
        {
            ...
            var hubOpen = EnvironmentKitHubWindow.IsOpen;
            if (hubOpen)
                EnvironmentKitHubWindow.NotifyBuildCompleted(title, shortMsg);
            else
                Debug.Log($"[CaveBuild] Completion (non-modal): {title}\n{shortMsg}");
            if (quality != null)
            {
