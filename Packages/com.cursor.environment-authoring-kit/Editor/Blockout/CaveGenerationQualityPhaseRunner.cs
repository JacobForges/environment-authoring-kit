    ...
    public static class CaveGenerationQualityPhaseRunner
    {
            ...
            out string summary)
        {
            ...
            for (var i = startInclusive; i < end; i++)
                RunSinglePhase(i, caveRoot, ground, request, log, ref ok, ref passed, end);

            FinalizeRangeLog(log, passed, startInclusive, end, out summary);
            if (!ok)
        ...
        public static void QueueRunRange(
            Transform caveRoot,
            SceneGroundInfo ground,
            WorldGenerationRequest request,
            int startInclusive,
            ...
            System.Action<bool, string> onComplete)
        {
            if (caveRoot == null || request == null)
            {
                onComplete?.Invoke(false, "Cave gen phases skipped — no cave.");
                return;
            }

            var end = Mathf.Min(startInclusive + count, PhaseCount);
            var state = new QueuedRangeState
            {
                CaveRoot = caveRoot,
                Ground = ground,
                Request = request,
                ...
                End = end,
                PhaseIndex = startInclusive,
                OnComplete = onComplete,
                Log = new StringBuilder(),
            };
            state.Log.AppendLine("{");
            state.Log.AppendLine($"  \"generatedUtc\": \"{System.DateTime.UtcNow:o}\",");
            state.Log.AppendLine("  \"phases\": [");
            ScheduleQueuedRangePhase(state);
        }

        static void ScheduleQueuedRangePhase(QueuedRangeState state)
        {
            if (state.PhaseIndex >= state.End)
            {
                FinalizeRangeLog(state.Log, state.Passed, state.StartInclusive, state.End, out var summary);
                if (!state.Ok)
                    Debug.LogWarning("[CaveBuild] " + summary);
                else
                    Debug.Log("[CaveBuild] " + summary);
                state.OnComplete?.Invoke(state.Ok, summary);
                return;
            }

            var phaseIndex = state.PhaseIndex;
            CaveBuildActionPacing.ScheduleBuildStep(
                ...
                CaveBuildPipelineDomains.QueueLabel($"cave gen quality — {PhaseIds[phaseIndex]}"),
                CaveBuildActionPacing.ActionWeight.Light);
        }

        static void RunSinglePhase(
            int i,
            Transform caveRoot,
            SceneGroundInfo ground,
            WorldGenerationRequest request,
