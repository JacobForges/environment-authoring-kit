...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    public static class CaveBuildEnhancementRunner
    {
        ...
        public static void RunHook(CaveBuildEnhancementCatalog.Hook hook, Transform caveRoot = null)
        {
            ...
            var phases = new List<CaveBuildEnhancementCatalog.PhaseDef>();
            ...
            QueueEnhancementHookPhase(hook, caveRoot, phases, 0);
        }

        static void QueueEnhancementHookPhase(
            CaveBuildEnhancementCatalog.Hook hook,
            Transform caveRoot,
            List<CaveBuildEnhancementCatalog.PhaseDef> phases,
            int index)
        {
            ...
            CaveBuildActionPacing.ScheduleLight(
                () =>
                {
                    ...
                    QueueEnhancementHookPhase(hook, caveRoot, phases, index + 1);
                },
                CaveBuildPipelineDomains.QueueLabel($"enhancement {hook} — {phase.id}"));
        }
            ...
            switch (phase.id)
            {
                ...
                case "creative_mountain_trail_ridges":
                    if (request != null)
                    {
                        ...
                        var env = UnityObjectCompat.FindAny<EnvironmentRoot>();
