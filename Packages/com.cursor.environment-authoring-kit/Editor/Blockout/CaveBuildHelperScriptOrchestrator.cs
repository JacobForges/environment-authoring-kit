...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildHelperScriptOrchestrator
    {
        ...
        {
            if (moment == Moment.PreBuildGateComplete &&
                ...
                !CaveBuildSurfaceCompletionGate.IsTerrainGradingComplete)
            {
                CaveBuildEditorLog.LogCave(
                    "[Helpers] Pre-build prompt export deferred — surface terrain grading in progress.",
                    forceUnityConsole: true);
                CaveBuildActionPacing.QueueWhenIdle(
                    () => Queue(moment, ctx, onComplete),
                    CaveBuildPipelineDomains.QueueLabel("pre-build prompts — after surface idle"));
                return;
            }
            ...
            CaveBuildActionPacing.QueueWhenIdle(
                ...
                CaveBuildPipelineDomains.QueueLabel($"helpers {moment} — idle handoff"));
        }
            ...
            Action<bool> onAwaitingTsx = null)
        {
            ...
            CaveBuildActionPacing.QueueWhenIdle(
                () => QueueStepAt(
                    ...
                    onAwaitingTsx),
                CaveBuildPipelineDomains.QueueLabel("helpers CavePhasePromptRefresh — idle handoff"));
        }
            ...
            CaveBuildActionPacing.ScheduleHeavyChain(
                () => RunOneStep(step, moment, (ok, msg, awaiting) =>
                {
                    ...
                    if (!ok)
                    {
                        ...
                        if (CaveBuildPhasePromptBridge.TryRecoverPhasePromptExportFailure(
                                hub,
                                step.Script,
                                ref msg))
                        {
