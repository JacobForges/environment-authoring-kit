...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceTerrainCraterRepair
    {
        ...
        /// <summary>Skip crater fill inside the primary cave mouth bowl (terrain ladder must not erase it).</summary>
        public static float ResolveMouthPreserveRadiusMeters() =>
            SurfacePrimaryMouthTerrainAuthor.PrimaryBowlRadiusMeters + 4f;
            ...
            float preserveCenterRadiusMeters = 0f)
        {
            ...
            CaveEditorUndo.RecordObject(data, "Repair terrain craters");
            ...
            float preserveCenterRadiusMeters = 0f)
        {
            ...
            var data = terrain.terrainData;
            CaveEditorUndo.RecordObject(data, "Repair terrain spikes");
            ...
            float preserveCenterRadiusMeters = 0f)
        {
            ...
            var data = terrain.terrainData;
            CaveEditorUndo.RecordObject(data, "Repair deep terrain bowls");
            ...
            float preserveCenterRadiusMeters = 0f)
        {
            ...
            var data = terrain.terrainData;
            CaveEditorUndo.RecordObject(data, "Repair mega terrain dips");
            ...
            float preserveCenterRadiusMeters = 0f)
        {
            ...
            var data = terrain.terrainData;
            CaveEditorUndo.RecordObject(data, "Repair wide terrain context");
        ...
        /// <summary>
        /// Paced ladder/post-prop repair — one heavy batch per tile (avoids 500+ row-scan queue steps on 513² maps).
        /// </summary>
            ...
            System.Action<int> onComplete)
        {
            ...
            var mouthPreserve = ResolveMouthPreserveRadiusMeters();
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    CaveBuildActionPacing.TouchQueueActivity();
                    var total = RepairHeightfieldLadderTileSync(terrain, centerWorld, extentMeters, mouthPreserve);
                    terrain.Flush();
                    onComplete?.Invoke(total);
                },
                CaveBuildPipelineDomains.QueueLabel("crater repair — tile batch"));
        }

        /// <summary>Fast ladder repair: de-checkerboard + blur + one crater/spike/mega pass (no 18× tail loop).</summary>
        public static int RepairHeightfieldLadderTileSync(
            Terrain terrain,
            Vector3 centerWorld,
            float extentMeters,
            float preserveCenterRadiusMeters)
        {
            ...
            var data = terrain.terrainData;
            CaveEditorUndo.RecordObject(data, "Ladder crater repair");
            var res = data.heightmapResolution;
            var heights = data.GetHeights(0, 0, res, res);

            var total = SurfaceTerrainHeightSmoothing.DeCheckerboardHeights(
                heights,
                res,
                terrain,
                centerWorld,
                extentMeters,
                strength: 0.42f);
            total += SurfaceTerrainRefinement.BoxBlurGraderSampleBandHeights(
                heights,
                res,
                terrain,
                centerWorld,
                extentMeters,
                blend: 0.32f);
            total += RepairCraterCellsHeights(
                heights,
                res,
                terrain,
                centerWorld,
                extentMeters,
                preserveCenterRadiusMeters);
            total += RepairSpikeCellsHeights(
                heights,
                res,
                terrain,
                centerWorld,
                extentMeters,
                preserveCenterRadiusMeters);
            total += RepairMegaDipCellsHeights(
                heights,
                res,
                terrain,
                centerWorld,
                extentMeters,
                preserveCenterRadiusMeters);
            total += RepairWideContextCellsHeights(
                heights,
                res,
                terrain,
                centerWorld,
        ...
        static int RepairCraterCellsHeights(
            float[,] heights,
            int res,
            Terrain terrain,
            Vector3 centerWorld,
            ...
            float preserveCenterRadiusMeters)
        {
            ...
            var innerFrac = preserveCenterRadiusMeters > 0f
                ? preserveCenterRadiusMeters / Mathf.Max(1f, extentMeters)
                : 0.08f;
            var fixedCount = 0;
        ...
        static int RepairSpikeCellsHeights(
            float[,] heights,
            int res,
            Terrain terrain,
            Vector3 centerWorld,
        ...
        static int RepairMegaDipCellsHeights(
            float[,] heights,
            int res,
            Terrain terrain,
            Vector3 centerWorld,
        ...
        static int RepairWideContextCellsHeights(
            float[,] heights,
            int res,
            Terrain terrain,
            Vector3 centerWorld,
            ...
            System.Action<int> onComplete)
        {
            ...
            if (res <= 513)
            {
                CaveBuildActionPacing.ScheduleHeavy(
                    () =>
                    {
                        var n = RepairCraterCells(terrain, centerWorld, extentMeters, preserveCenterRadiusMeters);
                        onComplete?.Invoke(n);
                    },
                    CaveBuildPipelineDomains.QueueLabel("crater repair — sync"));
                return;
            }
        ...
        static int RepairRowChunk(int res)
        {
            if (CaveBuildEditorResponsiveness.IsLongBuildActive)
                return res >= 1025 ? 32 : res >= 513 ? 128 : 256;
            return res >= 1025 ? 4 : res >= 513 ? 8 : 24;
        }

        static int RepairScanRowChunk(int res)
        {
            if (CaveBuildEditorResponsiveness.IsLongBuildActive)
                return res >= 1025 ? 64 : res >= 513 ? 256 : 512;
        ...
        static void RunRepairStep(RepairSession session, RepairStep step)
        {
            ...
            switch (step)
            {
                case RepairStep.Prepare:
                    CaveEditorUndo.RecordObject(session.Terrain.terrainData, "Repair terrain craters");
                    session.Heights = session.Terrain.terrainData.GetHeights(0, 0, session.Res, session.Res);
                    session.InnerFrac = session.PreserveCenterRadiusMeters > 0f
                        ? session.PreserveCenterRadiusMeters / Mathf.Max(1f, session.ExtentMeters)
                        : 0.08f;
                    session.ScanY = RingOuter;
                    session.RowY = 0;
                    CaveBuildActionPacing.ScheduleLight(
                        () => RunRepairStep(session, RepairStep.ScanRows),
                        CaveBuildPipelineDomains.QueueLabel("crater repair — scan"));
                    break;

                case RepairStep.ScanRows:
                {
                    var yEnd = Mathf.Min(session.Res - RingOuter, session.ScanY + RepairScanRowChunk(session.Res));
                    for (var y = session.ScanY; y < yEnd; y++)
                    {
                    ...
                    session.ScanY = yEnd;
                    CaveBuildActionPacing.TouchQueueActivity();
                    if (session.ScanY < session.Res - RingOuter)
                    {
                        CaveBuildRunStatusPublisher.PulseSubOperation(
                            "crater repair",
                            $"scan rows {session.ScanY}/{session.Res}");
                        CaveBuildActionPacing.ScheduleLight(
                            () => RunRepairStep(session, RepairStep.ScanRows),
                            CaveBuildPipelineDomains.QueueLabel("crater repair — scan rows"));
                        return;
                    }
                    ...
                    CaveBuildActionPacing.ScheduleLight(
                        ...
                        CaveBuildPipelineDomains.QueueLabel("crater repair — upload"));
                    break;
                }
        ...
        static void FlushRepairRows(RepairSession session, int yStart, int yEnd)
        {
            ...
            CaveBuildTerrainHeightmapMemory.ApplyHeightSlice(session.Terrain, 0, yStart, slice, requestDelayLod: false);
        }
    }
}
#endif
