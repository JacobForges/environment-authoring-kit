...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildQualityStageFixer
    {
            ...
            out string actionTaken)
        {
            ...
            {
                case "path":
                    changed = FixPath(caveRoot, request, adventure, seed, out actionTaken);
                    // fl-fgs-ofms104-lidar-karst-geomorph: spline bootstrap must re-seat root Y (void depth) without moving locked XZ.
                    if (ground != null && ground.HasAnchor &&
                        !CaveGroundPlacementUtility.IsRootPlacementAcceptable(caveRoot, ground))
                    {
                        var depthChanged = CaveGroundPlacementUtility.TrySnapMouthToSurfaceDepthOnly(
                            caveRoot, ground, out var depthSnap);
                        if (!depthChanged &&
                            Mathf.Abs(CaveGroundPlacementUtility.MeasureRootDepthError(caveRoot, ground)) >
                            CaveGroundPlacementUtility.MaxVerticalErrorMeters * 2f)
                        {
                            depthChanged = CaveGroundPlacementUtility.RecoverRootFromExcessiveDepth(
                                caveRoot, ground, out depthSnap);
                        }

                        // fl-fgs-ofms104-lidar-karst-geomorph: path bootstrap can precede entrance marker — full locked repair.
                        if (!depthChanged &&
                            !CaveGroundPlacementUtility.IsRootPlacementAcceptable(caveRoot, ground))
                        {
                            depthChanged = CaveGroundPlacementUtility.TryRepairLockedGroundPlacement(
                                caveRoot, ground, out depthSnap);
                        }

                        if (depthChanged)
                        {
                            actionTaken = string.IsNullOrEmpty(actionTaken) ? depthSnap : actionTaken + "; " + depthSnap;
                            changed = true;
                        }
                    }
        ...
        /// <summary>
        /// ea-seed-leveraging-llms-efficient-failure-analysis: recover pinned seed from memory, graded JSON, or EditorPrefs.
        /// </summary>
        static int ResolveSessionSeed(WorldGenerationRequest request, int sessionSeedFallback = 0)
        {
            if (request != null && request.Seed > 0)
                return request.Seed;

            if (sessionSeedFallback > 0)
                return sessionSeedFallback;

            if (CaveBuildQualitySystem.LastGradedReport?.Seed > 0)
                return CaveBuildQualitySystem.LastGradedReport.Seed;

            var disk = CaveBuildRungPromptExporter.TryLoadQualityReport();
            if (disk?.Seed > 0)
                return disk.Seed;

            return CaveBuildWorkflowCoordinator.TryReadLastSeed(out var sessionSeed) ? sessionSeed : 0;
        }

        static bool InferAdventureHybrid(Transform caveRoot, bool adventure)
        {
            if (adventure)
                return true;

            var audit = CaveBuildVisualShellAuditor.Audit(caveRoot);
            return audit.HasRouteTerrainFloor;
        }

        /// <summary>
        /// ea-seed-leveraging-llms-efficient-failure-analysis: RouteTerrain partial builds can lose metadata while
        /// visual_shell still passes — recover seed from the last graded report or session roll.
        /// </summary>
        static bool TryEnsureBuildMetadataFromSession(
            Transform caveRoot,
            WorldGenerationRequest request,
            bool adventure,
            int sessionSeedFallback,
            out CaveBuildMetadata meta)
        {
            meta = caveRoot?.GetComponent<CaveBuildMetadata>();
            if (meta != null)
                return true;

            var audit = CaveBuildVisualShellAuditor.Audit(caveRoot);
            var hybrid = InferAdventureHybrid(caveRoot, adventure);
            var seed = ResolveSessionSeed(request, sessionSeedFallback);
            if (seed <= 0)
                return false;

            if (request != null)
            {
                var segments = request.CaveTunnelSegments;
                var chambers = request.CaveChamberCount;
                if (audit.HasRouteTerrainFloor && audit.SolutionPathSteps > 0)
                {
                    segments = Mathf.Max(segments, audit.SolutionPathSteps + 2);
                    chambers = Mathf.Max(chambers, Mathf.Clamp(Mathf.RoundToInt(segments * 0.28f), 2, 6));
                }

                meta = caveRoot.gameObject.AddComponent<CaveBuildMetadata>();
                CaveEditorUndo.RegisterCreated(meta, "Cave Build Metadata");
                meta.Set(seed, segments, chambers, hybrid);
                meta.mazeGenFlavor = request.MazeGenFlavor;
                return true;
            }

            if (!audit.HasRouteTerrainFloor && !adventure)
                return false;

            meta = caveRoot.gameObject.AddComponent<CaveBuildMetadata>();
            CaveEditorUndo.RegisterCreated(meta, "Cave Build Metadata");
            // fl-fgs-ofms104-lidar-karst-geomorph: infer maze scale from route shell, not map photocopy.
            var inferredSegments = Mathf.Max(10, audit.SolutionPathSteps > 0 ? audit.SolutionPathSteps + 2 : 12);
            var inferredChambers = Mathf.Clamp(Mathf.RoundToInt(inferredSegments * 0.28f), 2, 6);
            meta.Set(seed, inferredSegments, inferredChambers, hybrid);
            return true;
        }

        /// <summary>
        /// Path rung bootstrap — regenerate knots from locked metadata without moving root XZ.
        /// MISSING_SPLINE playbook.
        /// </summary>
        // [bot:rung:path:2026-06-07]
        static bool TryBootstrapSplinePathFromMetadata(
            Transform caveRoot,
            WorldGenerationRequest request,
            bool adventure,
            int sessionSeedFallback,
            out CaveSplinePathAuthoring authoring,
            out string action)
        {
            authoring = caveRoot?.GetComponent<CaveSplinePathAuthoring>();
            action = string.Empty;

            if (authoring != null && authoring.Knots != null && authoring.Knots.Count >= 4)
                return false;

            if (!TryEnsureBuildMetadataFromSession(caveRoot, request, adventure, sessionSeedFallback, out var meta))
            {
                action = "Missing CaveSplinePathAuthoring and no build metadata to regenerate path.";
                return false;
            }

            var metaSeed = meta.seed;
            var tunnelSegs = meta.tunnelSegments;
            var chambers = meta.chamberCount;
            var flavor = meta.mazeGenFlavor;

            CaveMazeLayout layout;
            if (adventure && request != null && request.UseLayoutPrototype)
                layout = CaveMazeLayoutGenerator.GeneratePrototype(metaSeed, tunnelSegs, chambers);
            else
                layout = CaveMazeLayoutGenerator.Generate(metaSeed, tunnelSegs, chambers, flavor);

            if (layout?.PathKnots == null || layout.PathKnots.Count < 4)
            {
                action = "Could not generate spline path knots from locked build metadata.";
                return false;
            }
            ...
            CaveEditorUndo.RecordObject(authoring, "Bootstrap Spline Path (path)");
            authoring.SetPath(layout.PathKnots, spline.TotalLength);
            action = $"Bootstrapped {layout.PathKnots.Count} spline path knots from metadata (seed {metaSeed}).";
            return true;
        }

        /// <summary>
        /// Compact RouteTerrain builds pass visual_shell while path knots and block rings were never stamped.
        /// arXiv:2603.06679 — cardinal block rings per solution cell for lane readability.
        /// </summary>
        static bool TryEnsureCompactRouteBlocksAfterPathBootstrap(
            Transform caveRoot,
            WorldGenerationRequest request,
            out string action)
        {
            action = string.Empty;
            if (caveRoot == null || !CaveCompactRouteUtility.NeedsCompactRouteDensityRepair(caveRoot))
                return false;
            ...
            var adventure = CaveGeometryPaths.IsAdventureCave(caveRoot);
            if (!TryEnsureBuildMetadataFromSession(caveRoot, request, adventure, 0, out var meta))
                return false;
            ...
            var chambers = meta.chamberCount;
            var flavor = meta.mazeGenFlavor;
            var layout = CaveMazeLayoutGenerator.Generate(seed, tunnelSegs, chambers, flavor);
            if (layout?.SolutionPath == null || layout.SolutionPath.Count < 2)
                return false;

            var rebuilt = audit.CaveBlockCount <= 0
                ? CaveCompactRouteUtility.RebuildCompactRouteShell(caveRoot, layout, seed)
                : CaveCompactRouteUtility.RebuildCompactBlockRingsOnly(caveRoot, layout, seed);
            if (rebuilt <= 0)
                return false;

            action =
                $"Rebuilt {rebuilt} compact route piece(s) on RouteTerrain " +
                $"(seed {seed} — OFMS104 void scale / lane readability).";
            return true;
        }

        static void AppendCompactRouteBlockRepair(
            Transform caveRoot,
            WorldGenerationRequest request,
            ref string action)
        {
            if (!TryEnsureCompactRouteBlocksAfterPathBootstrap(caveRoot, request, out var blockAction))
                return;

            action = string.IsNullOrEmpty(action) ? blockAction : action + "; " + blockAction;
        }
            ...
            out string action)
        {
            ...
            // MISSING_SPLINE playbook — FixPath → TryBootstrapSplinePathFromMetadata; adventure → TryBootstrapMissingTrue3DShell.
            // [bot:rung:path:2026-06-07]
            var authoring = caveRoot.GetComponent<CaveSplinePathAuthoring>();
            if (authoring == null || authoring.Knots == null || authoring.Knots.Count < 4)
            {
                TryBootstrapSplinePathFromMetadata(
                    caveRoot, request, adventure, sessionSeedFallback, out authoring, out action);

                authoring = caveRoot.GetComponent<CaveSplinePathAuthoring>();
                // [bot:rung:path:2026-06-07] MISSING_SPLINE — compact RouteTerrain counts as adventure hybrid.
                if (InferAdventureHybrid(caveRoot, adventure) &&
                    (authoring == null || authoring.Knots == null || authoring.Knots.Count < 4) &&
                    TryBootstrapMissingTrue3DShell(
                        caveRoot, request, sessionSeedFallback, out var shellAction, forPathRepair: true))
                {
                    action = string.IsNullOrEmpty(action) ? shellAction : action + "; " + shellAction;
                }

                // [bot:rung:path:2026-06-07] MISSING_SPLINE — grader requires ≥4 knots; do not fall through to descent-only.
                authoring = caveRoot.GetComponent<CaveSplinePathAuthoring>();
                if (authoring == null || authoring.Knots == null || authoring.Knots.Count < 4)
                {
                    action = string.IsNullOrEmpty(action)
                        ? "Missing CaveSplinePathAuthoring or too few path knots."
                        : action;
                    return false;
                }

                AppendCompactRouteBlockRepair(caveRoot, request, ref action);
                return true;
            }

            var routeAudit = CaveBuildVisualShellAuditor.Audit(caveRoot);
            if (routeAudit.HasRouteTerrainFloor &&
                CaveCompactRouteUtility.NeedsCompactRouteDensityRepair(caveRoot))
            {
                AppendCompactRouteBlockRepair(caveRoot, request, ref action);
            ...
            out string action)
        {
            ...
            if (adventure)
            {
                // ea-seed-leveraging-llms-efficient-failure-analysis: visual_shell can pass while knots + maze shell were never built.
                if (NeedsTrue3DShellBootstrap(caveRoot, request) &&
                    TryBootstrapMissingTrue3DShell(caveRoot, request, 0, out action))
                    return true;

                if (TryBootstrapSplinePathFromMetadata(caveRoot, request, true, 0, out _, out action))
                    return true;
        ...
        static bool FixBlockTunnel(Transform caveRoot, int seed, bool adventure, out string action)
        {
            ...
            if (adventure)
            {
                // arXiv:2603.06679 — sparse compact route (0 blocks) must rebuild rings, not only cap renderers.
                if (CaveCompactRouteUtility.NeedsCompactRouteDensityRepair(caveRoot) &&
                    TryEnsureCompactRouteBlocksAfterPathBootstrap(caveRoot, null, out action))
                    return true;
        ...
        {
            if (UnityObjectCompat.FindAny<Terrain>() == null)
            {
        ...
        {
            if (request != null)
                return request.UseTrue3DCaveSystem || CaveGeometryPaths.IsAdventureCave(caveRoot);

            // Partial builds keep CaveBuildMetadata before adventure shells exist — still expect true-3D maze.
            if (caveRoot != null &&
                (caveRoot.GetComponent<CaveBuildMetadata>() != null || CaveGeometryPaths.IsAdventureCave(caveRoot)))
                return true;
        ...
        static bool NeedsTrue3DShellBootstrap(Transform caveRoot, WorldGenerationRequest request)
        {
            ...
            if (caveRoot.GetComponent<CaveBuildMetadata>() != null || request != null)
                return true;

            return ResolveSessionSeed(request) > 0;
        }
        ...
        static bool TryBootstrapMissingTrue3DShell(
            ...
            int sessionSeedFallback,
            out string action,
            bool forPathRepair = false)
        {
            action = string.Empty;
            var needsShell = NeedsTrue3DShellBootstrap(caveRoot, request);
            // [bot:rung:path:2026-06-07] MISSING_SPLINE — path deep repair must run when knots missing even if shell exists.
            if (!forPathRepair && !needsShell)
                return false;

            var adventure = CaveGeometryPaths.IsAdventureCave(caveRoot);
            if (!TryEnsureBuildMetadataFromSession(caveRoot, request, adventure, sessionSeedFallback, out var meta))
            {
                action = "No build metadata or session seed for true-3D shell bootstrap.";
                return false;
            }
            ...
            var knotsOk = authoring.Knots != null && authoring.Knots.Count >= 4;
            if (forPathRepair && !needsShell)
            {
                if (knotsOk)
                {
                    action =
                        $"Bootstrapped {authoring.Knots.Count} spline path knots from metadata (seed {metaSeed}).";
                    return true;
                }

                action = "Could not bootstrap spline path knots from locked build metadata.";
                return false;
            }
            ...
            var useAdventureHybrid = request?.UseBlockTunnel ?? adventure;
            ...
            knotsOk = authoring.Knots != null && authoring.Knots.Count >= 4;
            action =
                $"Bootstrapped true-3D shell from metadata (seed {metaSeed}): {placed} maze piece(s), {ribs} rib(s), {shellPieces} route shell piece(s).";
            if (placed > 0 || ribs > 0 || shellPieces > 0)
                return true;

            // fl-fgs-ofms104-lidar-karst-geomorph: path knots must exist before void-depth snap / geometry sampling.
            if (knotsOk)
            {
                action =
                    $"Bootstrapped {authoring.Knots.Count} spline path knots (seed {metaSeed}); " +
                    "maze shell meshes pending next meat pass.";
                return true;
            }
            ...
            out string action)
        {
            ...
            if (TryBootstrapMissingTrue3DShell(caveRoot, request, 0, out action))
            {
                CaveSceneMaterialRepair.RepairCaveRoot(caveRoot);
                CaveEnclosureShellBuilder.HideRoutePlatformSlabs(caveRoot);
                return true;
        ...
        static bool FixOrganicMesh(Transform caveRoot, int seed, bool adventure, out string action)
        {
            ...
            if (adventure)
            {
                ...
                // arXiv:2603.06679 — compact route needs cardinal block rings per solution cell, not an empty void.
                if (CaveCompactRouteUtility.NeedsCompactRouteDensityRepair(caveRoot))
                {
                    var meta = caveRoot.GetComponent<CaveBuildMetadata>();
                    if (meta != null)
                    {
                        var layout = CaveMazeLayoutGenerator.Generate(
                            meta.seed, meta.tunnelSegments, meta.chamberCount, meta.mazeGenFlavor);
                        if (layout?.SolutionPath != null && layout.SolutionPath.Count >= 2)
                        {
                            var rebuilt = CaveCompactRouteUtility.RebuildCompactBlockRingsOnly(
                                caveRoot, layout, meta.seed);
                            if (rebuilt > 0)
                            {
                                action =
                                    $"Purged {n} layered piece(s); rebuilt {rebuilt} compact block ring(s) " +
                                    $"(sparse route — OFMS104 void scale / lane readability).";
                                return true;
                            }
                        }
                    }
