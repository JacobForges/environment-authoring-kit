...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class LavaTubeMaterialUpgrader
    {
        ...
        static int UpgradeAllPackMaterialsInternal(bool forceMenuLog)
        {
            ...
            foreach (var root in LavaTubePrefabCatalog.GetModuleAssetRoots())
            {
                ...
                foreach (var guid in guids)
                {
                    ...
                    if (mat == null || ShouldSkipMaterialUpgrade(mat))
                        continue;
        ...
        /// <summary>UI, TMP, legacy particles, etc. — never bulk-convert to URP Lit (avoids _BaseMap console spam).</summary>
        public static bool ShouldSkipMaterialUpgrade(Material mat)
        {
            if (mat == null)
                return true;

            var shaderName = mat.shader != null ? mat.shader.name : string.Empty;
            if (!string.IsNullOrEmpty(shaderName) && IsNonUpgradeableShader(shaderName))
                return true;

            return IsNonUpgradeableMaterialPath(AssetDatabase.GetAssetPath(mat));
        }

        public static bool IsNonUpgradeableMaterialPath(string path)
        {
        ...
        static bool IsNonUpgradeableShader(string shaderName)
        {
            ...
            return shaderName.StartsWith("GUI/", StringComparison.Ordinal) ||
                   shaderName.StartsWith("TextMeshPro/", StringComparison.Ordinal) ||
                   ...
                   shaderName.Contains("Particles/", StringComparison.Ordinal);
        }

        public static bool UpgradeMaterial(Material mat, Shader urpLit)
        {
            if (mat == null || urpLit == null || ShouldSkipMaterialUpgrade(mat))
                return false;
            ...
            var hasBaseMap = mat.HasProperty("_BaseMap");
            var needsTextures = albedo != null && hasBaseMap && mat.GetTexture("_BaseMap") != albedo;
            var missingBase = hasBaseMap && mat.GetTexture("_BaseMap") == null && albedo != null;
        ...
        public static bool IsBrokenMaterial(Material mat)
        {
            ...
            if (ShouldSkipMaterialUpgrade(mat))
                return false;
