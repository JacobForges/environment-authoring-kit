...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildFullRunPreflight
    {
            ...
            bool writeMarkdown = true)
        {
            ...
            SurfaceTerrainTileExpansion.EnsureDefaultGridAnchors(ground);

            Add(list, "ground_anchor", "Ground tag / anchor", ground.HasAnchor
                ? Severity.Pass
                : Severity.Block, ground.HasAnchor
                ? $"Anchor: {ground.Anchor?.name ?? "ok"}"
                : "Empty scene: kit creates tagged Ground + PortalFive on first build.");

            var hasGridHost = GameObject.Find(SurfaceTerrainTileExpansion.FlatTerrainHostName) != null;
            Add(
                list,
                "grid_anchors",
                "Surface grid hosts (FlatHost + GridAnchor)",
                hasGridHost ? Severity.Pass : Severity.Warn,
                hasGridHost
            ...
            if (fullWorld && request != null && request.PreserveBlockoutGridLayout)
            {
                Add(
                    list,
                    "terrain_first_layout",
                    "FullWorld terrain-first (no modular Grid layout)",
                    Severity.Block,
                    "PreserveBlockoutGridLayout must be false — see ResearchCache category fullworld_do_not.");
            }
            else if (fullWorld && ground.HasAnchor &&
                     string.Equals(ground.Anchor.name, "Grid", StringComparison.OrdinalIgnoreCase) &&
                     ground.Terrain == null)
            {
                Add(
                    list,
                    "terrain_first_ground",
                    "Ground anchor not on terrain",
                    Severity.Warn,
                    "Ground=Grid — tag SurfaceTerrainMain as Ground before build, or let the kit rebind after terrain create (fullworld_do_not).");
            }
            else if (fullWorld)
            {
                Add(
                    ...
                    "FullWorld terrain-first (no modular Grid layout)",
                    Severity.Pass,
                    request != null && !request.PreserveBlockoutGridLayout
                        ? "PreserveBlockoutGridLayout=false; expect ~289 terrain tiles (17×17) or 81-tile core when UseExtendedOpenWorldGrid=false."
                        : "FullWorld scope.");
            }
            ...
            Add(list, "ladder_cave_layout", $"Ladder cave_layout for seed {seed}",
                ladderComplete || !settings.useIncrementalLadder ? Severity.Pass : Severity.Warn,
                ladderComplete
                    ? "Prior completion exists — incremental ON is OK after one full pass."
                    : !settings.useIncrementalLadder
                        ? "Incremental off — Full AAA Rebuild runs a fresh ladder."
        ...
        /// <summary>
        /// Fixes preflight WARN items that would block a clean unattended FullWorld / Full AAA Rebuild.
        /// Call before <see cref="Run"/> when invalidateEntireLadder is true.
        /// </summary>
        public static string ApplyUnattendedFullWorldRemediations(
            ...
            bool invalidateEntireLadder)
        {
            ...
            if (settings.useIncrementalLadder)
            {
                ...
                fixes.Add("useIncrementalLadder=false (fresh ladder for Full AAA Rebuild)");
            }
