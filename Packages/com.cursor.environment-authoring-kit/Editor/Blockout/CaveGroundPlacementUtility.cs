...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveGroundPlacementUtility
    {
        ...
        public static void QueueFinalizeGroundPlacement(
            Transform caveRoot,
            SceneGroundInfo ground,
            int seed,
            System.Action<bool> onComplete) =>
            CaveBuildActionPacing.ScheduleLight(
                () => onComplete?.Invoke(FinalizeGroundPlacement(caveRoot, ground, out _, seed)),
                CaveBuildPipelineDomains.QueueLabel("ground — finalize placement"));

        public static void QueueTryRepairLockedGroundPlacement(
            Transform caveRoot,
            SceneGroundInfo ground,
            System.Action<bool> onComplete) =>
            CaveBuildActionPacing.ScheduleLight(
                () => onComplete?.Invoke(TryRepairLockedGroundPlacement(caveRoot, ground, out _)),
                CaveBuildPipelineDomains.QueueLabel("ground — repair locked placement"));

        public static void QueueEnsureFullyBuriedUnderSurface(
            Transform caveRoot,
            SceneGroundInfo ground,
            System.Action<bool> onComplete) =>
            CaveBuildActionPacing.ScheduleLight(
                () => onComplete?.Invoke(EnsureFullyBuriedUnderSurface(caveRoot, ground, out _)),
                CaveBuildPipelineDomains.QueueLabel("ground — burial envelope"));

        public static void QueueReseatCaveUnderTerrainAfterSurface(
            Transform caveRoot,
            SceneGroundInfo ground,
            System.Action<bool> onComplete) =>
            CaveBuildActionPacing.ScheduleLight(
                () => onComplete?.Invoke(ReseatCaveUnderTerrainAfterSurface(caveRoot, ground, out _)),
                CaveBuildPipelineDomains.QueueLabel("ground — reseat after surface"));
    }
}
