...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceNavMeshBaker
    {
        ...
        public static void QueueBakePhase(
            Transform environmentRoot,
            Terrain terrain,
            Transform surfaceRoot,
            System.Action<bool, string> onComplete) =>
            QueueBakePhase(environmentRoot, terrain, surfaceRoot, null, onComplete);

        sealed class NavBakeSession
        {
            public Transform EnvironmentRoot;
            public Terrain MainTerrain;
            public Transform SurfaceRoot;
            public IReadOnlyList<Vector3> TrailWaypoints;
            public System.Action<bool, string> OnComplete;
            public List<NavMeshBuildSource> Sources = new();
            public Bounds Bounds;
            public List<Terrain> TerrainTiles = new();
            public int TerrainTileIndex;
            public bool MeshSourcesAdded;
        }
        ...
        public static void QueueBakePhase(
            ...
            System.Action<bool, string> onComplete)
        {
            if (environmentRoot == null || terrain == null)
            {
                onComplete?.Invoke(false, "No environment root or terrain.");
                return;
            }

            var session = new NavBakeSession
            {
                ...
                Bounds = new Bounds(environmentRoot.position, Vector3.one * 40f),
            };
            session.TerrainTiles.AddRange(SurfaceTerrainPlayRegion.CollectSurfaceTerrains(terrain));
            QueueNavBakeTerrainTile(session, 0);
        }

        static void QueueNavBakeTerrainTile(NavBakeSession session, int tileIndex)
        {
            if (session == null)
                return;

            if (tileIndex >= session.TerrainTiles.Count)
            {
                CaveBuildActionPacing.ScheduleHeavy(
                    () =>
                    {
                        if (!session.MeshSourcesAdded && session.SurfaceRoot != null)
                        {
                            AppendWalkableMeshSourcesFromRoot(
                                session.Sources,
                                ref session.Bounds,
                                session.SurfaceRoot);
                            session.MeshSourcesAdded = true;
                        }

                        QueueNavBakeFinalize(session);
                    },
                    CaveBuildPipelineDomains.SurfaceQueueLabel("navmesh — trail meshes"));
                return;
            }

            var terrain = session.TerrainTiles[tileIndex];
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    ...
                    if (terrain?.terrainData != null)
                    {
                        session.Sources.Add(new NavMeshBuildSource
                        {
                            shape = NavMeshBuildSourceShape.Terrain,
                            sourceObject = terrain.terrainData,
                            transform = Matrix4x4.TRS(terrain.transform.position, Quaternion.identity, Vector3.one),
                        ...
                        session.Bounds.Encapsulate(
                            new Bounds(
                                terrain.transform.position + terrain.terrainData.size * 0.5f,
                                terrain.terrainData.size));
                    }

                    QueueNavBakeTerrainTile(session, tileIndex + 1);
                },
                CaveBuildPipelineDomains.SurfaceQueueLabel(
                    $"navmesh — terrain tile {tileIndex + 1}/{session.TerrainTiles.Count}"));
        }

        static void QueueNavBakeFinalize(NavBakeSession session)
        {
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    CaveBuildRunStatusPublisher.PulseSubOperation("terrain phase", "NavMesh bake");
                    var ok = FinishNavBakeFromSession(session, out var message);
                    CaveBuildActionPacing.ScheduleLight(
                        () => session.OnComplete?.Invoke(ok, message),
                        CaveBuildPipelineDomains.SurfaceQueueLabel("navmesh — complete"));
                },
                CaveBuildPipelineDomains.SurfaceQueueLabel("navmesh — build data"));
        }

        static bool FinishNavBakeFromSession(NavBakeSession session, out string message)
        {
            message = null;
            if (session.EnvironmentRoot == null)
            {
            ...
            settings.agentClimb = session.TrailWaypoints != null && session.TrailWaypoints.Count > 0 ? 0.75f : 0.5f;
            settings.agentSlope = session.TrailWaypoints != null && session.TrailWaypoints.Count > 0 ? 48f : 42f;

            EncapsulateTrailWaypoints(ref session.Bounds, session.TrailWaypoints);
            session.Bounds.Expand(24f);
            ...
            var terrainSources = session.TerrainTiles.Count;
            var meshSources = session.Sources.Count - terrainSources;
            message =
                $"Surface NavMesh phase ({session.Sources.Count} sources: {terrainSources} terrain tile(s), {meshSources} trail/road mesh).";
