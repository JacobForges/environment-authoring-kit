...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    static class CaveLayoutTerrainPad
    {
        ...
        static void FlattenPadAlongRoute(Terrain terrain, Transform caveRoot, CaveMazeLayout layout)
        {
            ...
            if (CaveBuildEditorResponsiveness.IsLongBuildActive)
            {
                CaveBuildMicroTerrainHeightmap.QueueMutateRowBands(
                    terrain,
                    "Layout Terrain Pad",
                    CaveBuildPipelineDomains.QueueLabel("layout terrain pad"),
                    (heights, rowStart, _, originBand, sizeBand) =>
                    {
                        foreach (var cell in layout.SolutionPath)
                        {
                            ...
                            StampFlatDisc(heights, res, sizeBand, originBand, world, radius, normalized, rowStart);
                        }
                    },
                    () => terrain.Flush());
                return;
            ...
            int rowStart = 0)
        {
            ...
            for (var z = z0; z <= z1; z++)
            {
                var bandZ = z - rowStart;
                if (bandZ < 0 || bandZ >= bandRows)
                    continue;
                ...
                for (var x = x0; x <= x1; x++)
                {
                    ...
                    var y = Mathf.Lerp(heights[bandZ, x], heightNormalized, 0.85f);
                    heights[bandZ, x] = Mathf.Min(heights[bandZ, x], y);
                }
            }
        }
    }
