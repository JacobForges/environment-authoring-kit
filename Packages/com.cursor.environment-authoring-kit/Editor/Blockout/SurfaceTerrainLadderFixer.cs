...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceTerrainLadderFixer
    {
                ...
                CaveBuildActionPacing.ScheduleHeavy(
                    () =>
                    {
                        ...
                        CaveBuildRunStatusPublisher.PulseSubOperation(
                            "terrain ladder",
                            $"crater repair play tile {currentIdx}/{terrains.Count} (mountains later)");

                        var mouthPreserve = SurfaceTerrainCraterRepair.ResolveMouthPreserveRadiusMeters();
                        var count = SurfaceTerrainCraterRepair.RepairHeightfieldLadderTileSync(
                            current,
                            center,
                            extent,
                            mouthPreserve);
            ...
            Action<bool, string> onComplete)
        {
            ...
            void RunNext()
            {
                if (tileIndex >= terrains.Count)
                {
                    onComplete(
                        totalCells > 0,
                        $"Outer-ring smoothed {totalCells} cells for playable slopes ({terrains.Count} tile(s), paced).");
                    return;
                }

                var terrain = terrains[tileIndex++];
                CaveBuildActionPacing.ScheduleHeavy(
                    () =>
                    {
                        if (terrain != null)
                    ...
                    CaveBuildPipelineDomains.SurfaceQueueLabel(
                        $"terrain ladder slopes tile {tileIndex}/{terrains.Count}"));
            }
        ...
        /// <summary>
        /// Grader band (8%–105%) de-checkerboard + outer-ring laplacian — meanAbsSlope is sampled in that
        /// annulus; far-outer pass (48%+) targets neighbor-tile seams without flattening the LiDAR core.
        /// </summary>
        static int ApplyPlayableSlopeOuterSmooth(Terrain terrain, Vector3 center, float extent)
        {
            ...
            var cells = SurfaceTerrainHeightSmoothing.DeCheckerboardOnTerrain(
                terrain, center, extent, strength: 0.34f);

            const float midAnnulusPreserve = 0.20f;
            const float farOuterPreserve = 0.48f;
            cells += SurfaceTerrainRefinement.SmoothOuterHeightRingPublic(
                terrain, center, extent, strength: 0.34f, preserveRadiusFraction: midAnnulusPreserve);
            cells += SurfaceTerrainRefinement.SmoothOuterHeightRingPublic(
                terrain, center, extent, strength: 0.28f);
            cells += SurfaceTerrainRefinement.SmoothOuterHeightRingPublic(
                terrain, center, extent, strength: 0.20f);
            cells += SurfaceTerrainRefinement.SmoothOuterHeightRingPublic(
                terrain, center, extent, strength: 0.26f, preserveRadiusFraction: farOuterPreserve);
            return cells;
        }
