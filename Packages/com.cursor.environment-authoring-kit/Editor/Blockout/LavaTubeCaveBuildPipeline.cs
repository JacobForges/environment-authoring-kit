...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static partial class LavaTubeCaveBuildPipeline
    {
        ...
        /// <summary>Place or move surface PlayerSpawnPoint to the play-disk grid center.</summary>
        public static void EnsurePlayDiskCenterSpawn(SceneGroundInfo ground)
        {
            if (ground == null)
                return;

            var caveRoot = CaveGeometryPaths.FindCaveSystemRoot();
            EnsureMainSceneSurfaceSpawn(caveRoot != null ? caveRoot.transform : null, ground);
        }
        ...
        internal sealed class QueuedPipelineContext
        {
            ...
            public int MonolithicChunk = -1;
            public LavaTubeCaveBuildReport MonolithicReport;
            public CaveMonolithicGeneratePacing.Session MonolithicSession;
            public bool AwaitingGenerationQuality;
            public int WorldStageSub;
            public bool AwaitingResearchStep;
        }
        ...
        {
            var ctx = _queued;
            if (ctx?.Request != null && CaveBuildHealOrchestrator.LooksLikeFailureReport(report))
            {
                CaveBuildHealOrchestrator.OnQueuedPipelineFailure(
                    report,
                    ctx.Request,
                    ctx.Ground,
                    ctx.CaveRoot,
                    ctx.Meat?.Quality ?? CaveBuildQualitySystem.LastGradedReport);
            }
