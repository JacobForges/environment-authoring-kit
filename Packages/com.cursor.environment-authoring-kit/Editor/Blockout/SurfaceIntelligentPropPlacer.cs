...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceIntelligentPropPlacer
    {
        ...
        public sealed class CategoryPlacementSession
        {
            ...
            public BiomePropCatalog BiomeCatalog;
            ...
            out string message)
        {
            ...
            if (!TryPlaceCategoryLadderPassChunk(
                    ...
                    category,
                    session,
                    DefaultPropsPerEditorChunk,
                    out _))
            {
                message = "Prop placement chunk made no progress.";
                return false;
            }
        ...
        /// <summary>Paced category pass — one heavy chunk per editor-queue step (tile/zone batches inside chunk).</summary>
        public static void QueueCategoryLadderPass(
            Transform surfaceRoot,
            Terrain terrain,
            Vector3 groundCenter,
            float extentMeters,
            ...
            System.Action<bool, string> onComplete)
        {
            ...
            if (!TryBeginCategoryPlacementSession(
                    ...
                    out var beginMsg))
            {
                onComplete(false, string.IsNullOrEmpty(beginMsg) ? "Could not start prop placement session." : beginMsg);
                return;
            }

            QueueCategoryLadderPassChunk(
                terrain,
                vegRoot,
                seed,
                category,
        ...
        static void QueueCategoryLadderPassChunk(
            Terrain terrain,
            Transform vegRoot,
            int seed,
            SurfacePropCategory category,
            ...
            System.Action<bool, string> onComplete)
        {
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    ...
                    {
                        onComplete(false, "Prop placement session missing.");
                        return;
                    }

                    var placedBefore = session.Placed;
                    TryPlaceCategoryLadderPassChunk(
                        terrain,
                        vegRoot,
                        seed,
                        category,
                    ...
                    {
                        var ok = TryFinalizeCategoryPlacementSession(
                            terrain,
                            vegRoot,
                            category,
                            session,
                            out var finalizeMsg);
                        onComplete(ok, finalizeMsg);
                        return;
                    }

                    if (session.Placed == placedBefore)
                    {
                        TryFinalizeCategoryPlacementSession(terrain, vegRoot, category, session, out var stallMsg);
                        onComplete(false, stallMsg);
                        return;
                    }

                    QueueCategoryLadderPassChunk(terrain, vegRoot, seed, category, session, onComplete);
                },
                CaveBuildPipelineDomains.SurfaceQueueLabel(
                    $"vegetation {category} chunk ({session.Placed}/{session.MaxCount})"));
        }
        ...
        public static bool TryBeginCategoryPlacementSession(
            ...
            out string message) =>
            TryBeginCategoryPlacementSession(
                surfaceRoot,
                terrain,
                groundCenter,
                extentMeters,
            ...
            out string message)
        {
            ...
            biomeCatalog ??= request != null ? BiomePropCatalog.Load(request) : BiomePropCatalog.Load();
            ...
            if (needsRebuild)
            {
                ...
                {
                    SurfaceTerrainPropPlacementRegion.CollectUnifiedSpreadPlacementSlotsForCategory(
            ...
            activeSession.BiomeCatalog ??= biomeCatalog;
            activeSession.PlayCenter = playCenter;
        ...
        /// <summary>
        /// Legacy second pass — pipeline now uses <see cref="SurfaceTerrainPropPlacementRegion.CollectUnifiedSpreadPlacementSlotsForCategory"/>.
        /// </summary>
        [Obsolete("Use unified full-map spread in the primary category pass.")]
            ...
            System.Random rng)
        {
            ...
            foreach (var slot in session.OrderedSlots)
            {
                ...
                if (!TryPlaceSlot(terrain, vegRoot, slot, pool, rng, category, session.PlanEntries, out _, out var resolved, session))
                    continue;
        ...
        static bool TryPlaceSlot(
            ...
            CategoryPlacementSession placementSession = null)
        {
            world = Vector3.zero;
            terrainName = slot.TerrainName;
            var slotPool = ResolvePoolForSlot(slot, pool, category, placementSession);
            if (slotPool == null || slotPool.Count == 0)
                return false;

            var prefab = slotPool[rng.Next(slotPool.Count)];
        ...
        static List<GameObject> ResolvePoolForSlot(
            PlacementSlot slot,
            List<GameObject> fallbackPool,
            SurfacePropCategory category,
            CategoryPlacementSession session)
        {
            if (session?.BiomeCatalog == null)
                return fallbackPool;

            var seed = session.Request?.Seed ?? 0;
            var blend = BiomePropFeatherResolver.ResolveAtWorld(
                slot.Position,
                session.PlayCenter,
                seed,
                session.Request);
            var pool = session.BiomeCatalog.BlendedPool(blend, category);
            return pool.Count > 0 ? pool : fallbackPool;
        }
        ...
        public sealed class SurfaceVegetationCatalog
        {
            ...
            public static SurfaceVegetationCatalog Load()
            {
                ...
                InjectCc0Vegetation(cat);
                return cat;
            }

            static void InjectCc0Vegetation(SurfaceVegetationCatalog cat)
            {
                for (var g = 1; g <= 6; g++)
                {
                    TryCc0(cat.Grass, $"G-G{g:D2}");
                    TryCc0(cat.Grass, $"G-C{g:D2}");
                    TryCc0(cat.Grass, $"G-T{g:D2}");
                }
