...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveBuildSurfaceCompletionGate
    {
        ...
        static bool _fullWorldGridPipelineActive;
        ...
        public static void ResetForNewBuildSession()
        {
            ...
            PipelineContentPreservePolicy.ResetForNewBuildSession();
            _surfaceBuildActive = false;
        ...
        public static void MarkFullWorldGridPipelineStarted(int tilePlanCount = 0)
        {
        ...
        public static void MarkFullWorldGridPipelineFinished(bool success)
        {
            _fullWorldGridPipelineActive = false;
        ...
        public static void MarkSurfaceBuildStarted()
        {
            ...
            if (main == null && ground?.HasAnchor == true)
                main = ground.Anchor.GetComponentInChildren<UnityEngine.Terrain>();
            if (main != null)
            {
                var request = WorldGenerationRequest.LoadOrDefault();
                CaveBuildHealOrchestrator.CaptureSessionBaseline(request?.Seed ?? 0, main, null);
            }

            CaveBuildPipelinePhaseTracker.OnSurfaceWorldStarted();
        ...
        public static void MarkTerrainGradingComplete(WorldGenerationRequest request)
        {
            ...
            CaveBuildPipelinePhaseTracker.OnTerrainGradingComplete();
        ...
        public static bool IsReadyForCaveMeatLoop(WorldGenerationRequest request, SceneGroundInfo ground)
        {
            ...
            if (_fullWorldGridPipelineActive)
                return false;
            ...
            SceneGroundInfo ground)
        {
            ...
            if (_fullWorldGridPipelineActive)
                parts.Add("fullWorldGridPipelineActive");
            ...
            bool surfaceWorldSuccess)
        {
            ...
            if (request.SurfaceScope == SurfaceBuildScope.FullWorld &&
                (_fullWorldGridPipelineActive || !_surfaceWorldGeneratorFinished))
            {
                LogState(
                    "handoff deferred — FullWorld grid/terraform still queued (cave must not start yet)");
                return;
            }
            ...
            if (request.SurfaceScope is SurfaceBuildScope.FullWorld or SurfaceBuildScope.SurfaceOnly &&
                !IsCompleteForSeed(request) &&
                HasUsableSurfaceInScene(ground) &&
                _surfaceWorldGeneratorFinished)
            {
                _completedSeed = request.Seed;
                _completedScope = request.SurfaceScope;
                _surfaceBuildActive = false;
        ...
        public static bool IsReadyForCave(WorldGenerationRequest request, SceneGroundInfo ground)
        {
            ...
            switch (request.SurfaceScope)
            {
                ...
                case SurfaceBuildScope.FullWorld:
                    ...
                    if (_surfaceBuildActive)
                        return false;
                    return IsCompleteForSeed(request) && HasUsableSurfaceInScene(ground);
                default:
                    return true;
            }
        }
        ...
        public static bool CanStartCaveGeometryNow(WorldGenerationRequest request, SceneGroundInfo ground)
        {
            ...
            if (request.SurfaceScope == SurfaceBuildScope.FullWorld)
                return IsReadyForCave(request, ground) && IsReadyForCaveMeatLoop(request, ground);
            ...
            bool surfaceWorldSuccess)
        {
            ...
            if (_fullWorldGridPipelineActive)
            {
                LogState("release handoff skipped — FullWorld grid pipeline still active");
                return;
            }
