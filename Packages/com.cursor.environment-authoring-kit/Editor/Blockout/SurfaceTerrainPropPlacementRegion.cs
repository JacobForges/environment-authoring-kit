...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    static class SurfaceTerrainPropPlacementRegion
    {
            ...
            WorldGenerationRequest request)
        {
            ...
            var terrains = CaveBuildAaaSessionPolicy.UsesExtendedOpenWorldGrid
            ...
            List<SurfaceIntelligentPropPlacer.PlacementSlot> slots)
        {
            ...
            AppendHillshadeDetectedPropSlots(
                slots,
                mainTerrain,
                playCenter,
                extentMeters,
        ...
        public static void CollectPlacementSlotsForCategory(
            ...
            SurfacePropCategory category,
            List<SurfaceIntelligentPropPlacer.PlacementSlot> slots) =>
            CollectUnifiedSpreadPlacementSlotsForCategory(
                mainTerrain, trailsRoot, playCenter, extentMeters, seed, category, slots);
        ...
        public const string SatelliteDetectionRel =
            "Assets/EnvironmentKit/Generated/SurfaceSatellitePropDetection.json";

        /// <summary>Satellite/hillshade pseudo–object detection → prop slots (never heightmap).</summary>
        static void AppendHillshadeDetectedPropSlots(
            List<SurfaceIntelligentPropPlacer.PlacementSlot> slots,
            Terrain mainTerrain,
            Vector3 playCenter,
            float extentMeters,
            ...
            SurfacePropCategory category)
        {
            if (mainTerrain == null || category == SurfacePropCategory.Rocks)
                return;

            var spacing = category switch
            {
                SurfacePropCategory.Trees => 5.8f,
                SurfacePropCategory.Bushes => 4.2f,
                SurfacePropCategory.Grass => 3.4f,
                SurfacePropCategory.GroundCover => 3.6f,
                _ => 5f,
            };

            var terrains = SurfaceTerrainPlayRegion.CollectSurfaceTerrains(mainTerrain);
            var rng = new System.Random(seed + 90210 + (int)category * 313);
            var maxSlots = Mathf.Min(
                category == SurfacePropCategory.Trees ? 2400 : 3200,
                TargetCountForCategory(category, terrains.Count));
            var added = 0;
            var classHits = new Dictionary<string, int>();

            foreach (var terrain in terrains)
            {
                if (terrain?.terrainData == null || added >= maxSlots)
                    break;

                var origin = terrain.transform.position;
                var size = terrain.terrainData.size;
                var cols = Mathf.Max(2, Mathf.FloorToInt(size.x / spacing));
                var rows = Mathf.Max(2, Mathf.FloorToInt(size.z / spacing));

                for (var row = 0; row <= rows && added < maxSlots; row++)
                {
                    for (var col = 0; col <= cols && added < maxSlots; col++)
                    {
                        var wx = origin.x + (col + 0.5f) / (cols + 1) * size.x;
                        var wz = origin.z + (row + 0.5f) / (rows + 1) * size.z;
                        if (!SurfaceTerrainPlayRegion.ContainsWorldXZ(terrain, wx, wz))
                            continue;

                        if (!SurfaceHillshadeObjectMask.TrySampleWorld(
                                wx,
                                wz,
                                playCenter,
                                extentMeters,
                                ...
                                out _,
                                out _) ||
                            !SurfaceHillshadeObjectMask.MatchesPropCategory(cls, category))
                            continue;

                        if (cls == SurfaceHillshadeObjectMask.SurfaceClass.Water)
                            continue;

                        if (!IsWalkableSlope(terrain, new Vector3(wx, playCenter.y, wz)))
                            continue;

                        var sector = (col + row * 7 + terrain.name.GetHashCode()) & 0x7fff;
                        var slotCategory = category switch
                        {
                            SurfacePropCategory.Trees => "tree",
                            SurfacePropCategory.Bushes => "bush",
                            SurfacePropCategory.Grass => "grass",
                            SurfacePropCategory.GroundCover => "flower",
                            _ => "mixed",
                        };

                        slots.Add(new SurfaceIntelligentPropPlacer.PlacementSlot
                        {
                            Position = new Vector3(wx, playCenter.y, wz),
                            SectorIndex = sector,
                            SpacingM = spacing,
                            YawDeg = rng.Next(0, 360),
                            Scale = category == SurfacePropCategory.Trees
                                ? 0.9f + (sector % 4) * 0.05f
                                : 0.65f + (sector % 4) * 0.06f,
                            Category = slotCategory,
                            TerrainName = terrain.name,
                        });
                        added++;
                        var key = cls.ToString();
                        classHits[key] = classHits.TryGetValue(key, out var n) ? n + 1 : 1;
                    }
                }
            }
            ...
            {
                CaveBuildEditorLog.LogSurface(
                    $"[Surface] Satellite layout — {added} {category} slot(s) from hillshade detection (class mask, not terrain).",
                    forceUnityConsole: true);
                WriteSatelliteDetectionSummary(seed, category, added, classHits);
            }
        }

        static void WriteSatelliteDetectionSummary(
            int seed,
            SurfacePropCategory category,
            int slotsAdded,
            Dictionary<string, int> classHits)
        {
            var hub = CaveBuildCursorSettings.ResolveHubRoot();
            var path = Path.Combine(hub, SatelliteDetectionRel);
            try
            {
                ...
                foreach (var kv in classHits)
                    hitLines.Add($"    \"{kv.Key}\": {kv.Value}");
                lines.Add(string.Join(",\n", hitLines));

                lines.Add("  },");
                lines.Add("  \"policy\": \"Dark/green hillshade pixels → props; LiDAR elev grid → macro terrain only.\"");
                lines.Add("}");
                File.WriteAllText(path, string.Join("\n", lines));
            }
            catch
            {
                // Non-fatal — placement still runs.
            }
        }
                ...
                for (var row = 0; row <= rows; row++)
                {
                    ...
                    for (var col = 0; col <= cols; col++)
                    {
                        ...
                        if (SurfaceHillshadeObjectMask.TrySampleWorld(
                                ...
                                out _,
                                out _) &&
                            SurfaceHillshadeObjectMask.MatchesPropCategory(hillCls, category))
                        {
                            continue;
                        }
