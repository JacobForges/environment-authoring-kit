    ...
    public static class CaveBuildPhasePromptBridge
    {
            ...
            bool skipManifestRebuild = false)
        {
            ...
            if (CaveBuildAaaSessionPolicy.ForcePerPhasePromptExport)
                extraEnvs.Add("CAVE_FORCE_PROMPT_EXPORT=1");
        ...
        public static bool ExportHollowTitanPhasePromptManifest(out string message) =>
            RunTsx(
                "generate-hollow-titan-phase-prompt-manifest.ts",
                null,
                null,
                60_000,
                out message);

        public static bool ExportBiomeLayoutPhasePromptManifest(out string message) =>
            RunTsx(
                "generate-biome-layout-phase-prompt-manifest.ts",
                null,
                null,
                60_000,
                out message);
            ...
            params string[] extraEnvs)
        {
            ...
            CaveBuildTsxProcessRunner.BeginRun(
                ...
                {
                    if (!ok &&
                        TryRecoverPhasePromptExportFailure(hub, scriptName, ref resultMsg))
                        ok = true;
                    if (ok)
            ...
            params string[] extraEnvs)
        {
            ...
            if (!ok)
            {
                if (TryRecoverPhasePromptExportFailure(hub, scriptName, ref message))
                    return true;
                return false;
            }
        ...
        /// <summary>Do not block FullWorld on tsx exit 1 when an active phase prompt already exists on disk.</summary>
        internal static bool TryRecoverPhasePromptExportFailure(string hub, string scriptName, ref string message)
        {
            if (scriptName != "generate-phase-prompts.ts")
                return false;

            var promptPath = Path.Combine(hub, ActivePhasePromptPath);
            if (!File.Exists(promptPath))
                return false;

            message =
                $"{scriptName} exited non-zero — continuing with existing {ActivePhasePromptPath}.";
            CaveBuildEditorLog.LogCaveWarning($"[Helpers] {message}");
            return true;
        }
