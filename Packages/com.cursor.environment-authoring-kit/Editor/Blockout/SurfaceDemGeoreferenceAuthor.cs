...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceDemGeoreferenceAuthor
    {
        ...
        const string PrefLastFloridaCounty = "CaveBuild_LastFloridaCountyId";
        const string PrefRandomizeFloridaCounty = "CaveBuild_RandomizeFloridaCountyEachBuild";

        /// <summary>When true (default), each FullWorld build picks a new panhandle county even if cave seed is pinned.</summary>
        public static bool RandomizeFloridaCountyEachBuild =>
            EditorPrefs.GetBool(PrefRandomizeFloridaCounty, true) ||
            EditorPrefs.GetBool("CaveBuild_RandomizeEachTime", true);

        static int HashCountyIndex(int salt, int count)
        {
            if (count <= 1)
                return 0;
        ...
        static int ResolveCountyPickSalt(int caveSeed)
        {
            if (RandomizeFloridaCountyEachBuild)
                return CaveLayoutRoll.NewSeed();

            if (CaveBuildDeterminism.IsPinned)
                return CaveBuildDeterminism.PinnedSeed;

            return caveSeed;
        }

        public static SurfaceLidarTerrainAuthor.CountyEntry PickCountyForSeed(
            ...
            int caveSeed)
        {
            ...
            var pickSalt = ResolveCountyPickSalt(caveSeed);
            var idx = HashCountyIndex(pickSalt, pool.Length);

            if (pool.Length > 1 && RandomizeFloridaCountyEachBuild)
            {
                var last = EditorPrefs.GetString(PrefLastFloridaCounty, string.Empty);
                if (!string.IsNullOrEmpty(last) &&
                    string.Equals(pool[idx].countyId, last, StringComparison.OrdinalIgnoreCase))
                {
                    idx = (idx + 1 + HashCountyIndex(pickSalt ^ 0x5f3759df, pool.Length - 1)) % pool.Length;
                }
            }

            var pick = pool[idx];
            EditorPrefs.SetString(PrefLastFloridaCounty, pick.countyId ?? string.Empty);

            var countyList = string.Join(", ", Array.ConvertAll(pool, c => c.countyId));
            CaveBuildEditorLog.LogSurface(
                $"[Surface] Florida LiDAR county — picked {pick.countyId} ({pool.Length} with segment: {countyList}). " +
                $"Cave seed={caveSeed}, county salt={pickSalt}, randomizeCounty={RandomizeFloridaCountyEachBuild}.",
            ...
            out string message)
        {
            ...
            if (!TryCreateStampSession(
                    ...
                    georef,
                    manifestRel,
                    SurfaceLidarGuidedSculptPolicy.PreferSculptOverStamp,
                    out var session,
                    out message))
                return false;
        ...
        /// <summary>Non-blocking LiDAR landscape sculpt — prep, row bands, and height upload are separate queue steps.</summary>
        public static void QueueApplyLidarGuidedLandscapeSculpt(
            Terrain terrain,
            Vector3 groundCenter,
            float extentMeters,
            int seed,
            Action<string> onComplete) =>
            QueueApplyGeoreferencedStampInternal(terrain, groundCenter, extentMeters, seed, onComplete, sculptOnly: true);

        /// <summary>Legacy entry — routes to LiDAR-guided sculpt when <see cref="SurfaceLidarGuidedSculptPolicy.PreferSculptOverStamp"/>.</summary>
        public static void QueueApplyGeoreferencedStamp(
            ...
            int seed,
            Action<string> onComplete) =>
            QueueApplyGeoreferencedStampInternal(
                terrain,
                groundCenter,
                extentMeters,
                seed,
                onComplete,
                sculptOnly: SurfaceLidarGuidedSculptPolicy.PreferSculptOverStamp);

        static void QueueApplyGeoreferencedStampInternal(
            Terrain terrain,
            Vector3 groundCenter,
            float extentMeters,
            int seed,
            ...
            bool sculptOnly)
        {
            ...
            if (!TryLoadGeorefForSeed(seed, out var georef, out var manifestRel))
            {
                CaveBuildActionPacing.ScheduleLightChain(
                    ...
                    CaveBuildPipelineDomains.SurfaceQueueLabel(
                        sculptOnly ? "surface LiDAR fallback sculpt" : "surface LiDAR fallback stamp"));
                return;
            }
            ...
            CaveBuildActionPacing.ScheduleLightChain(
                () => RunStampPrepare(pending, sculptOnly),
                CaveBuildPipelineDomains.SurfaceQueueLabel(
                    sculptOnly ? "LiDAR landscape sculpt prep" : "LiDAR DEM prep"));
        }
        ...
        sealed class StampSession : IDisposable
        {
            ...
            public string BuildResultMessage()
            {
                ...
                var message =
                    $"LiDAR landscape sculpt ({Georef.CountyId}, {scaleNote}) — {Changed} cells " +
                    $"(procedural carve/rise @ ~{SurfaceTerrainLidarCreativeGuide.CharacterHeightMeters:F1}m character scale + " +
                    $"≤{SurfaceTerrainLidarCreativeGuide.MaxGuideInfluence:P0} LiDAR structural bias + research brief).";
                if (UseElev)
                    message += " Elev grid drives ridge rise / valley carve (smoothed macro, not photocopy).";
                else if (Georef.HasCloseUpSegment)
                    message +=
                        " (No elevation-grid.json — hillshade carve/rise only; npm run sync-florida-hillshades -- --elev-grid=128)";
        ...
        static void RunStampPrepare(StampPendingWork pending, bool sculptOnly)
        {
            if (pending?.Terrain == null)
            {
                pending?.OnComplete?.Invoke("LiDAR sculpt aborted.");
                return;
            }
        ...
        static int StampRowChunkSize(int res)
        {
            if (CaveBuildEditorResponsiveness.IsLongBuildActive)
                return res >= 1025 ? 32 : res >= 513 ? 96 : 128;
            return res >= 1025 ? 4 : res >= 513 ? 6 : 12;
        }

        static int StampRowsPerEditorTick()
        {
            if (!CaveBuildEditorResponsiveness.IsLongBuildActive)
                return 1;
            return SurfaceTerrainTileExpansion.PreferSequentialFullWorldTerrain ? 4 : 2;
        }
        ...
        static void RunStampRows(StampSession session)
        {
            ...
            var ticks = StampRowsPerEditorTick();
            var yEnd = session.RowY;

            for (var tick = 0; tick < ticks && yEnd < session.Res; tick++)
            {
                var yStart = yEnd;
                yEnd = Mathf.Min(session.Res, yStart + chunk);
                StampGeorefRowRange(session, yStart, yEnd);
            }

            if (session.RowY == 0 || yEnd >= session.Res || yEnd % (chunk * 8) < chunk)
            {
                CaveBuildLiveSceneFeedback.NotifySurfacePhase(
                    session.SculptOnly
                        ? $"LiDAR sculpt rows {yEnd}/{session.Res} ({session.Georef.CountyId})…"
        ...
        static void RunStampHeightmapUpload(StampSession session)
        {
            ...
            CaveBuildLiveSceneFlushUtility.FlushWorldView(session.Terrain);
        ...
        static void FlushHeightRows(StampSession session, int yStart, int yEnd)
        {
            ...
            CaveBuildTerrainHeightmapMemory.ApplyHeightSlice(session.Terrain, 0, yStart, slice, requestDelayLod: true);
        }
            ...
            out string message)
        {
            ...
            var authoritative = georef.HasCloseUpSegment && !sculptOnly;
            ...
            if (!useElev)
            {
                ...
                try
                {
                    ...
                    BuildHillshadeCaches(tex, out luminance, out hillshadeRgb, out hillshadeW, out hillshadeH);
                }
                finally
                {
                    if (tex != null)
            ...
            session = new StampSession
            {
                ...
                LidarStrength = sculptOnly
                    ? SurfaceLidarGuidedSculptPolicy.MaxLidarGuideInfluence
                    : SurfaceTerrainLidarCreativeGuide.MaxGuideInfluence,
        ...
        static void BuildHillshadeCaches(
            Texture2D tex,
            out float[] luminance,
            out Color[] rgb,
            out int width,
            out int height)
        {
            ...
            luminance = new float[pixels.Length];
            rgb = new Color[pixels.Length];
        ...
        static Color SampleHillshadeColor(StampSession session, float u, float v)
        {
            if (session.HillshadeRgb == null || session.HillshadeWidth < 2 || session.HillshadeHeight < 2)
                return new Color(SampleHillshadeLuminance(session, u, v), SampleHillshadeLuminance(session, u, v), SampleHillshadeLuminance(session, u, v));
            ...
            Color At(int ix, int iy) => session.HillshadeRgb[iy * w + ix];
            return Color.Lerp(
                Color.Lerp(At(x0, y0), At(x1, y0), tx),
                Color.Lerp(At(x0, y1), At(x1, y1), tx),
                ty);
        }
        ...
        static void StampGeorefRowRange(StampSession session, int yStart, int yEnd)
        {
            ...
            for (var y = yStart; y < yEnd; y++)
            {
                for (var x = 0; x < res; x++)
                {
                    ...
                    if (dist > session.ExtentMeters * 0.52f)
                        guideWeight *= 0.12f;
                    ...
                    {
                        var rgb = SampleHillshadeColor(session, uv.x, uv.y);
                        ...
                        guideNorm = SurfaceTerrainLidarCreativeGuide.SampleStructuralGuideNormFromHillshade(
                            ...
                            session.AnchorNorm,
                            session.Size.y,
                            session.TargetRelief * 0.45f);
                    }

                    var sculptedNorm = SurfaceTerrainLidarCreativeGuide.ApplyLandscapeCarveAndRise(
                        creativeNorm,
                        guideNorm,
                        session.Size.y,
                        guideWeight,
                    ...
                    var blendStrength = session.Authoritative && !session.SculptOnly
                        ? Mathf.Lerp(
                            0.58f,
                            SurfaceTerrainLidarCreativeGuide.PlayDiskBlendStrength * 0.92f,
                            guideWeight)
                        : guideWeight * session.LidarStrength;

                    var innerPreserve = dist < session.PreserveR * 1.05f
                        ? Mathf.Clamp01(dist / Mathf.Max(session.PreserveR, 0.5f)) * 0.22f
                        : 0f;
                    session.Heights[y, x] = Mathf.Clamp01(
                        Mathf.Lerp(
                            Mathf.Lerp(before, creativeNorm, innerPreserve),
                            sculptedNorm,
                            Mathf.Clamp01(blendStrength)));
