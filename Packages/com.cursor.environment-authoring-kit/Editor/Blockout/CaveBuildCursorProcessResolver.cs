...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    static class CaveBuildCursorProcessResolver
    {
        ...
        /// <summary>Run an arbitrary tsx script under Tools/cave-grader (e.g. demo-recap-narrate.ts).</summary>
        public static bool TryCreateTsxScript(
            string hubRoot,
            string scriptPath,
            string extraArgs,
            out ProcessStartInfo psi,
            out string error,
            string workflow = "demo_recap")
        {
            psi = null;
            error = null;
            ...
            if (File.Exists(tsxCli))
            {
                ...
                ApplyEnvironment(psi, hubRoot, null, workflow);
                return true;
            }
        ...
        /// <summary>Unity must invoke .py scripts with python3 — not node.</summary>
        public static bool TryResolvePython3(out string pythonPath, out string error)
        {
            error = null;
            foreach (var name in new[] { "python3", "python" })
            {
                ...
                if (TryWhich(name, out pythonPath))
                    return true;
            }

            pythonPath = null;
            error = "python3 not found. Install Python 3 (brew install python) for recap compose.";
            return false;
        }

        public static bool TryRunPythonScript(
            string hubRoot,
            ...
            string workflow = "demo_recap")
        {
            ...
            if (!TryResolvePython3(out var pythonPath, out error))
                return false;

            var toolsDir = Path.Combine(hubRoot, CaveBuildCursorAgentBridge.ToolsRelativePath);
            var script = Path.Combine(toolsDir, scriptFileName);
            if (!File.Exists(script))
            {
                error = "Missing script: " + scriptFileName;
                return false;
            }
            ...
            try
            {
                ...
                if (proc.ExitCode != 0)
                {
                    error = string.IsNullOrEmpty(stderr) ? $"exit {proc.ExitCode}" : stderr.Trim();
                    return false;
                }
