...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public sealed class CaveBuildCursorSettings : ScriptableObject
    {
        ...
        [Header("Build video & LiDAR sculpt")]
        [Tooltip("LiDAR DEM/hillshade guides carve/rise sculpt — never authoritative heightmap stamp replacement.")]
        public bool lidarGuidedSculptOnly = true;

        [Tooltip("Smooth Scene-view camera follows the active terrain tile (cinematographer framing).")]
        public bool cinematicSceneCamera = true;

        [Tooltip("Scene-view framing pull-back multiplier (1 = default, higher = wider).")]
        public float liveSceneCameraZoomOut = 3f;

        [Tooltip("While demo recording, force live Scene placement + camera follow even if stabilization is on.")]
        public bool forceLivePreviewWhenRecording = true;
        ...
        [Header("Quality fast gates")]
        [Tooltip("Fail-fast tile count, require Hollow Titan on surface, skip redundant autonomous loops when grade already good.")]
        public bool enableQualityFastGates = true;

        [Tooltip("Skip post-build autonomous Cursor loop when last CaveBuildQualityReport for this seed already meets fastGateMinOverallScore.")]
        public bool skipAutonomousWhenGradeAtOrAbove = true;

        [Tooltip("Overall score threshold for skipping autonomous ship loop (70–98).")]
        public int fastGateMinOverallScore = 88;

        [Tooltip("Re-place massive Hollow Titan on surface if missing before FullWorld terraform.")]
        public bool requireHollowTitanOnSurface = true;

        [Header("Hollow Titan landmark")]
        [Tooltip("12-step phased meat build (site pick → patrol nodes) with live scene feedback.")]
        public bool hollowTitanPhasedBuild = true;

        [Tooltip("Meters above terrain SampleHeight for tree root base.")]
        public float hollowTitanBaseClearanceMeters = 0.35f;

        [Tooltip("Interior floor count for hollow tree landmark.")]
        public int hollowTitanFloorCount = 7;

        [Tooltip("Vertical spacing between interior floors (meters).")]
        public float hollowTitanFloorHeightMeters = 9f;

        [Tooltip("Landmark-only enemy spawn markers per floor.")]
        public int hollowTitanEnemiesPerFloor = 2;

        [Tooltip("Landmark-only loot spawn markers per floor.")]
        public int hollowTitanLootPerFloor = 1;

        [Tooltip("Interior patrol nodes per floor.")]
        public int hollowTitanPatrolNodesPerFloor = 3;
        ...
        [Header("Auto-heal checkpoints")]
        [Tooltip(
            "Capture terrain + Generated JSON snapshots; promote good states; rollback and retry on failure " +
            "using the active AI provider from Hub Settings (Cursor, Ollama, local VLM, etc.).")]
        public bool enableFullAutoHeal = true;

        [Tooltip("Minimum overall score to promote a checkpoint as last-good rollback head.")]
        public int healMinScoreForGoodCheckpoint = 80;

        [Tooltip("Max rollback + fix + retry cycles per build session.")]
        public int maxHealRecoveryAttempts = 5;

        [Tooltip("Restore last-good terrain heightmaps + Generated artifacts before procedural/AI retry.")]
        public bool healRollbackBeforeRetry = true;

        [Tooltip("After rollback, invoke grade-and-fix via the active Hub AI provider.")]
        public bool healInvokeAiAfterRollback = true;

        [Tooltip("Seconds to wait after heal fixes before scheduling rebuild.")]
        public float healRetryDelaySeconds = 6f;
        ...
        [Header("3D Gaussian Splat (optional hero)")]
        [Tooltip("After surface build, place one hero splat anchor at the primary cave mouth. Off by default — GPU heavy.")]
        public bool enableGaussianSplatHeroAtCaveMouth;

        [Tooltip("Project path to GaussianSplatAsset (e.g. Assets/EnvironmentKit/GaussianSplats/hero.asset).")]
        public string gaussianSplatAssetPath = "";

        public GaussianSplatHeroSlot.QualityTier gaussianSplatHeroQuality =
            GaussianSplatHeroSlot.QualityTier.Medium;

        [Tooltip("Show splats in Editor Scene view (expensive with full terrain). Play Mode still uses gaussianSplatRenderInPlayMode.")]
        public bool gaussianSplatRenderInEditMode;

        [Tooltip("Show splats in Play Mode / builds.")]
        public bool gaussianSplatRenderInPlayMode = true;
        ...
        public void LoadFromPrefs()
        {
            ...
            lidarGuidedSculptOnly = EditorPrefs.GetBool(
                "CaveBuild_LidarGuidedSculptOnly",
                lidarGuidedSculptOnly);
            cinematicSceneCamera = EditorPrefs.GetBool(
                "CaveBuild_CinematicSceneCamera",
                cinematicSceneCamera);
            liveSceneCameraZoomOut = EditorPrefs.GetFloat(
                "CaveBuild_LiveSceneCameraZoomOut",
                liveSceneCameraZoomOut);
            forceLivePreviewWhenRecording = EditorPrefs.GetBool(
                "CaveBuild_ForceLivePreviewWhenRecording",
                forceLivePreviewWhenRecording);
            ...
            enableQualityFastGates = EditorPrefs.GetBool(
                "CaveBuild_EnableQualityFastGates", enableQualityFastGates);
            skipAutonomousWhenGradeAtOrAbove = EditorPrefs.GetBool(
                "CaveBuild_SkipAutonomousWhenGradeAtOrAbove", skipAutonomousWhenGradeAtOrAbove);
            fastGateMinOverallScore = EditorPrefs.GetInt(
                "CaveBuild_FastGateMinOverallScore", fastGateMinOverallScore);
            requireHollowTitanOnSurface = EditorPrefs.GetBool(
                "CaveBuild_RequireHollowTitanOnSurface", requireHollowTitanOnSurface);
            hollowTitanPhasedBuild = EditorPrefs.GetBool(
                "CaveBuild_HollowTitanPhasedBuild", hollowTitanPhasedBuild);
            hollowTitanBaseClearanceMeters = EditorPrefs.GetFloat(
                "CaveBuild_HollowTitanBaseClearance", hollowTitanBaseClearanceMeters);
            hollowTitanFloorCount = EditorPrefs.GetInt(
                "CaveBuild_HollowTitanFloorCount", hollowTitanFloorCount);
            hollowTitanFloorHeightMeters = EditorPrefs.GetFloat(
                "CaveBuild_HollowTitanFloorHeight", hollowTitanFloorHeightMeters);
            hollowTitanEnemiesPerFloor = EditorPrefs.GetInt(
                "CaveBuild_HollowTitanEnemiesPerFloor", hollowTitanEnemiesPerFloor);
            hollowTitanLootPerFloor = EditorPrefs.GetInt(
                "CaveBuild_HollowTitanLootPerFloor", hollowTitanLootPerFloor);
            hollowTitanPatrolNodesPerFloor = EditorPrefs.GetInt(
                "CaveBuild_HollowTitanPatrolPerFloor", hollowTitanPatrolNodesPerFloor);
            ...
            enableFullAutoHeal = EditorPrefs.GetBool("CaveBuild_EnableFullAutoHeal", enableFullAutoHeal);
            healMinScoreForGoodCheckpoint = EditorPrefs.GetInt(
                "CaveBuild_HealMinGoodScore", healMinScoreForGoodCheckpoint);
            maxHealRecoveryAttempts = EditorPrefs.GetInt(
                "CaveBuild_MaxHealRecoveryAttempts", maxHealRecoveryAttempts);
            healRollbackBeforeRetry = EditorPrefs.GetBool(
                "CaveBuild_HealRollbackBeforeRetry", healRollbackBeforeRetry);
            healInvokeAiAfterRollback = EditorPrefs.GetBool(
                "CaveBuild_HealInvokeAiAfterRollback", healInvokeAiAfterRollback);
            healRetryDelaySeconds = EditorPrefs.GetFloat(
                "CaveBuild_HealRetryDelaySeconds", healRetryDelaySeconds);
        ...
        public void SaveToPrefs()
        {
            ...
            EditorPrefs.SetBool("CaveBuild_LidarGuidedSculptOnly", lidarGuidedSculptOnly);
            EditorPrefs.SetBool("CaveBuild_CinematicSceneCamera", cinematicSceneCamera);
            EditorPrefs.SetFloat("CaveBuild_LiveSceneCameraZoomOut", liveSceneCameraZoomOut);
            EditorPrefs.SetBool("CaveBuild_ForceLivePreviewWhenRecording", forceLivePreviewWhenRecording);
            ...
            EditorPrefs.SetBool("CaveBuild_EnableQualityFastGates", enableQualityFastGates);
            EditorPrefs.SetBool("CaveBuild_SkipAutonomousWhenGradeAtOrAbove", skipAutonomousWhenGradeAtOrAbove);
            EditorPrefs.SetInt("CaveBuild_FastGateMinOverallScore", fastGateMinOverallScore);
            EditorPrefs.SetBool("CaveBuild_RequireHollowTitanOnSurface", requireHollowTitanOnSurface);
            EditorPrefs.SetBool("CaveBuild_HollowTitanPhasedBuild", hollowTitanPhasedBuild);
            EditorPrefs.SetFloat("CaveBuild_HollowTitanBaseClearance", hollowTitanBaseClearanceMeters);
            EditorPrefs.SetInt("CaveBuild_HollowTitanFloorCount", hollowTitanFloorCount);
            EditorPrefs.SetFloat("CaveBuild_HollowTitanFloorHeight", hollowTitanFloorHeightMeters);
            EditorPrefs.SetInt("CaveBuild_HollowTitanEnemiesPerFloor", hollowTitanEnemiesPerFloor);
            EditorPrefs.SetInt("CaveBuild_HollowTitanLootPerFloor", hollowTitanLootPerFloor);
            EditorPrefs.SetInt("CaveBuild_HollowTitanPatrolPerFloor", hollowTitanPatrolNodesPerFloor);
            ...
            EditorPrefs.SetBool("CaveBuild_EnableFullAutoHeal", enableFullAutoHeal);
            EditorPrefs.SetInt("CaveBuild_HealMinGoodScore", healMinScoreForGoodCheckpoint);
            EditorPrefs.SetInt("CaveBuild_MaxHealRecoveryAttempts", maxHealRecoveryAttempts);
            EditorPrefs.SetBool("CaveBuild_HealRollbackBeforeRetry", healRollbackBeforeRetry);
            EditorPrefs.SetBool("CaveBuild_HealInvokeAiAfterRollback", healInvokeAiAfterRollback);
            EditorPrefs.SetFloat("CaveBuild_HealRetryDelaySeconds", healRetryDelaySeconds);
            ...
            EditorPrefs.SetBool("CaveBuild_GaussianSplatEditMode", gaussianSplatRenderInEditMode);
            EditorPrefs.SetBool("CaveBuild_GaussianSplatPlayMode", gaussianSplatRenderInPlayMode);
        }
