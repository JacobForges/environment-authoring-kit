...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    static class CaveBuildActionPacing
    {
        ...
        static bool _whenIdlePolling;
        static Action _whenIdlePending;
        static string _whenIdleLabel;
        static double _whenIdleGraceUntil;
        static double _whenIdleWaitStartedAt;
        ...
        const double WhenIdleTsxWatchdogSeconds = 45.0;
        ...
        public static void TouchQueueActivity()
        {
            ...
            if (ArmedBatch.Count > 0 && !string.IsNullOrEmpty(ArmedBatch[0].Label))
                CaveBuildEditorQueueSafeguard.NotifyQueueProgress(ArmedBatch[0].Label);
        }
        ...
        public static void EmergencyAbortAll()
        {
            ...
            _whenIdlePolling = false;
            _whenIdlePending = null;
            _whenIdleLabel = null;
            _whenIdleGraceUntil = 0;
            _whenIdleWaitStartedAt = 0;
            EditorApplication.update -= PollWhenIdle;
        }

        /// <summary>
        /// Runs <paramref name="onComplete"/> only after the editor queue and active tsx processes are idle.
        /// </summary>
        public static void QueueWhenIdle(Action onComplete, string label = null)
        {
            if (onComplete == null)
                return;

            if (!IsBusy && !CaveBuildTsxProcessRunner.IsRunning)
            {
                ScheduleLight(onComplete, label ?? CaveBuildPipelineDomains.QueueLabel("pipeline — idle proceed"));
                return;
            }

            _whenIdlePending = onComplete;
            _whenIdleLabel = label ?? CaveBuildPipelineDomains.QueueLabel("pipeline — wait idle");
            _whenIdleWaitStartedAt = EditorApplication.timeSinceStartup;
            EnsureWhenIdlePolling();
        }

        static void EnsureWhenIdlePolling()
        {
            if (_whenIdlePolling)
                return;

            _whenIdlePolling = true;
            EditorApplication.update += PollWhenIdle;
        }

        static void PollWhenIdle()
        {
            if (_whenIdlePending == null)
            {
                _whenIdlePolling = false;
                EditorApplication.update -= PollWhenIdle;
                _whenIdleWaitStartedAt = 0;
            ...
            var queueBusy = IsBusy;
            var tsxBusy = CaveBuildTsxProcessRunner.IsRunning;
            if (queueBusy || tsxBusy)
            {
                _whenIdleGraceUntil = 0;
                var nowBusy = EditorApplication.timeSinceStartup;
                if (_whenIdleWaitStartedAt <= 0)
                    _whenIdleWaitStartedAt = nowBusy;

                if (!queueBusy &&
                    tsxBusy &&
                    nowBusy - _whenIdleWaitStartedAt >= WhenIdleTsxWatchdogSeconds)
                {
                    CaveBuildEditorLog.LogCaveWarning(
                        $"[Queue] QueueWhenIdle watchdog — cancelling stale tsx after {WhenIdleTsxWatchdogSeconds:F0}s: " +
                        $"{_whenIdleLabel}");
                    CaveBuildTsxProcessRunner.CancelActive("QueueWhenIdle watchdog timeout");
                }
            ...
            if (_whenIdleGraceUntil <= 0)
            {
                _whenIdleGraceUntil = now + 0.06;
                return;
            }

            if (now < _whenIdleGraceUntil)
                return;

            var next = _whenIdlePending;
            var label = _whenIdleLabel;
            _whenIdlePending = null;
            _whenIdleLabel = null;
            _whenIdlePolling = false;
            EditorApplication.update -= PollWhenIdle;
            ScheduleLight(next, label);
        }
        ...
        static void Enqueue(Action action, string label, ActionWeight weight, bool immediate = false)
        {
            ...
            if (!CaveBuildEditorQueueSafeguard.TryAcceptEnqueue(label, out var safeguardReason))
            {
                CaveBuildPipelineDomains.LogCaveWarning(safeguardReason);
                return;
            }
        ...
        static void Poll()
        {
            if (CaveBuildPauseController.ShouldHoldQueue())
                return;
        ...
        static void RunArmedBatchOnce()
        {
            ...
            finally
            {
                _insideQueueInvoke = false;
                _heavyRunning = false;
                CaveBuildStepCounter.Advance(item.Label);
            }
        ...
        static void FinishBatchRun(ActionWeight batchWeight)
        {
            ...
            _batchArmedAt = 0;
            for (var i = 0; i < ArmedBatch.Count; i++)
                CaveBuildEditorQueueSafeguard.OnDequeued(ArmedBatch[i].Label);
            ArmedBatch.Clear();
            _batchRunIndex = 0;
            _actionRunning = false;
            _heavyRunning = false;
        ...
        static double StallWatchdogLimitForLabel(string label)
        {
            ...
            if (label.IndexOf("crater", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                ...
                label.IndexOf("seam", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("mountain", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("terraform", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("play-band", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("labyrinth", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("pre-build", System.StringComparison.OrdinalIgnoreCase) >= 0 ||
                label.IndexOf("startup", System.StringComparison.OrdinalIgnoreCase) >= 0)
