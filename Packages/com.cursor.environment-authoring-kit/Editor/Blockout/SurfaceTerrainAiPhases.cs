...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class SurfaceTerrainAiPhases
    {
        ...
        static void PipelineWatchdogTick()
        {
            ...
            if (req.SurfaceScope == SurfaceBuildScope.FullWorld &&
        ...
        public static readonly string[] PhaseTitles =
        {
            ...
            "Terrain phase 5 — LiDAR-guided sculpt (carve/rise, not heightmap stamp)",
        ...
        internal sealed class QueueState
        {
            ...
            public BiomePropCatalog BiomePropsCatalog;
            ...
            public bool TerrainContractCheckDone;
            ...
            Action<bool, string> onComplete)
        {
            ...
            CaveBuildPipelinePhaseTracker.OnTerrainAiStarted();
            var state = new QueueState
            {
                ...
                Center = center,
                Extent = extent,
                MaxLadderIterations = duringCaveBuildSurface && fullWorld ? 4 : duringCaveBuildSurface ? 3 : 8,
            ...
            CaveBuildTerrainHeightmapMemory.ResetDelayLodBudget();
        ...
        static void AdvanceTerrainPhase(QueueState state)
        {
            CaveBuildActionPacing.QueueWhenIdle(
                () => SchedulePhaseStep(state),
                CaveBuildPipelineDomains.SurfaceQueueLabel("terrain phases — wait idle"));
        }

        static void SchedulePhaseStep(QueueState state)
        {
            if (!state.TerrainContractCheckDone && state.Ground?.Terrain != null && state.Request != null)
            {
                state.TerrainContractCheckDone = true;
                CaveBuildTerrainContractCheck.QueueAtTerrainPipelineStart(
                    state.Ground,
                    state.Request,
                    () => SchedulePhaseStep(state));
                return;
            ...
            if (index == 4)
            {
                ...
                {
                    CaveBuildEditorLog.LogSurface(
                        "[Surface] Terrain phase 5 — skipped (nine-tile DEM + seam lock already finished in surface build).",
                        forceUnityConsole: true);
                    state.PhasesOk = true;
                    AppendPhaseLog(
                        state,
                        index,
                        true,
                        "Skipped — play disk already locked in surface finish.",
                        "FullWorld nine-tile done");
                    state.PhaseIndex++;
                    AdvanceTerrainPhase(state);
                    return;
                }
                ...
                if (SurfaceFloridaDemBuildState.AuthoritativeStampCompletedThisBuild)
                {
                    ...
                    {
                        CaveBuildEditorLog.LogSurface(
                            $"[Surface] Terrain phase 5 — play-disk seam stitch on {tileCount} tile(s) (no DEM re-stamp at Ground anchor).",
                            forceUnityConsole: true);
                        CaveBuildActionPacing.ScheduleLight(
                            () =>
                            {
                                var stitched = SurfaceTerrainTileExpansion.StitchAllPlayDiskSeamsSync(
                                ...
                                AppendPhaseLog(
                                    ...
                                    index,
                                    state.PhasesOk,
                                    $"Play-disk seam stitch ({stitched} tile(s)) — main + neighbors, no wrong-center DEM.",
        ...
        static void RunTerrainPhaseBody(QueueState state, int index, string title)
        {
            ...
            if (lightGate && (index == 1 || index == 2))
            {
                var skipMsg =
                    "Skipped — FullWorld play disk skips terrain phase trail/water benches (mountain pipeline owns outer ring).";
                state.PhasesOk = true;
                AppendPhaseLog(state, index, true, skipMsg, "FullWorld play disk protected");
                CaveBuildEditorLog.LogSurface($"[Surface] {title} — {skipMsg}", forceUnityConsole: true);
                state.PhaseIndex++;
                AdvanceTerrainPhase(state);
                return;
            }
        ...
        static void RunTerrainPhaseNavMesh(QueueState state)
        {
            if (state.Ground?.Terrain == null)
            {
                Complete(state, false, "Terrain phases aborted — terrain missing.");
            ...
            if (EnvironmentKitHardwareBudget.Active.ConserveGpuMemory)
            {
                state.PhasesOk &= true;
                var skipMsg =
                    "NavMesh bake skipped in phase 6 — laptop/demo budget (terrain ladder defers full bake).";
                AppendPhaseLog(state, index, true, skipMsg, gateMsg);
                CaveBuildEditorLog.LogSurface($"[Surface] {title} — {skipMsg}", forceUnityConsole: true);
                CaveBuildSurfaceProgress.CompleteTerrainPhase(index);
                state.PhaseIndex++;
                AdvanceTerrainPhase(state);
            ...
            var envTransform = state.EnvRoot != null ? state.EnvRoot.transform : null;
            var surfaceRoot = ResolveSurfaceRoot(state);
            SurfaceNavMeshBaker.QueueBakePhase(
                envTransform,
                state.Ground.Terrain,
                surfaceRoot,
                (ok, phaseMsg) =>
                {
                    state.PhasesOk &= ok;
                    AppendPhaseLog(state, index, ok, phaseMsg ?? string.Empty, gateMsg);
                    CaveBuildEditorLog.LogSurface(
                        $"[Surface] {title} — {(ok ? "OK" : "issues")}: {phaseMsg}",
                        forceUnityConsole: true);
                    CaveBuildSurfaceProgress.CompleteTerrainPhase(index);
                    state.PhaseIndex++;
                    AdvanceTerrainPhase(state);
        ...
        internal static void ContinueAfterTerrainMeatLoop(QueueState state)
        {
            if (state == null)
                return;
            state.LadderIteration = 0;
            CaveBuildSurfaceProgress.Show("Primary cave mouth terrain (pre-props)");
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    SurfacePrimaryMouthTerrainAuthor.TryApply(state.Ground, state.Request, out var mouthMsg);
                    if (!string.IsNullOrEmpty(mouthMsg))
                    {
                        CaveBuildEditorLog.LogSurface(
                            $"[Surface] Pre-props mouth refresh — {mouthMsg}",
                            forceUnityConsole: true);
                    }

                    ScheduleSurfacePropsStep(state);
                },
                CaveBuildPipelineDomains.SurfaceQueueLabel("primary mouth terrain before props"));
        }
        ...
        static void SchedulePropsPlanStep(QueueState state)
        {
            ...
            BiomeLayoutConceptCatalog.LogPresetPrompt(state.Request);
            BiomeLayoutConceptCatalog.ExportPhasePromptManifest(out var manifestMsg);
            if (!string.IsNullOrEmpty(manifestMsg))
                CaveBuildEditorLog.LogSurface("[BiomeLayout] " + manifestMsg, forceUnityConsole: false);

            var presetIdx = Mathf.Clamp(state.Request?.ConceptLayoutIndex ?? 0, 0, BiomeLayoutConceptCatalog.PresetCount - 1);
            var promptBlock = BiomeLayoutConceptCatalog.FormatPresetPromptBlock(presetIdx, state.Request);
            CaveBuildEditorLog.LogSurface(
                $"[BiomeLayout] Prop phase prompt (preset {presetIdx}):\n{promptBlock}",
            ...
            CaveBuildActionPacing.ScheduleHeavyChain(
                ...
                CaveBuildPipelineDomains.SurfaceQueueLabel("surface props — unified spread"));
        }
        ...
        static void ScheduleSurfacePropCategory(QueueState state)
        {
            ...
            CaveBuildActionPacing.ScheduleHeavy(
                () =>
                {
                    ...
                    var vegCount = vegRoot != null ? vegRoot.childCount : 0;
                    if (CaveBuildTerrainFingerprint.ShouldSkipPropCategory(
                            cat.ToString(),
                            state.Ground.Terrain,
                            state.Request.Seed,
                            vegCount,
                    ...
                    {
                        CaveBuildEditorLog.LogSurface(
                            $"[Surface] Props {cat} — skipped ({propSkip}).",
                            forceUnityConsole: false);
                    ...
                    if (session.IsComplete)
                    {
                        ...
                        state.PropPlacementSession = null;
                        CaveBuildPropSnapRepair.QueueAfterPropCategory(
                            state.Ground,
                            state.Request,
                            cat.ToString(),
                            () =>
                ...
                CaveBuildPipelineDomains.SurfaceQueueLabel($"terrain vegetation spread {cat}"));
        }

        static void ScheduleLadderStep(QueueState state)
        {
            if (state.LadderReport != null &&
                CaveBuildGradingProfile.IsTerrainMeatPipelineMode(state.LadderReport.GradingMode))
            {
        ...
        static void SchedulePhase0OuterSmooth(QueueState state)
        {
            ...
            {
                var phaseMsg =
                    "Skipped — FullWorld 9-tile play disk is Florida LiDAR + seam lock only (no phase-0 edge smooth).";
                state.PhasesOk = true;
                CaveBuildEditorLog.LogSurface("[Surface] Terrain phase 1 — " + phaseMsg, forceUnityConsole: true);
                AppendPhaseLog(state, 0, true, phaseMsg, "FullWorld play disk protected");
                state.PhaseIndex++;
                AdvanceTerrainPhase(state);
                return;
            }
        ...
        static void Complete(QueueState state, bool ok, string summary)
        {
            ...
            {
                if (!ok && CaveBuildHealOrchestrator.IsEnabled)
                {
                    var caveRoot = UnityEngine.Object.FindAnyObjectByType<CaveBuildMetadata>()?.transform;
                    CaveBuildHealOrchestrator.OnBuildFailure(
                        "terrain_pipeline_failed",
        ...
        static void FinishTerrainPipelineComplete(QueueState state, bool ok, string summary)
        {
            ...
            if (cave != null)
            {
                ...
                CaveRouteMouthSnapUtility.TrySyncEntranceToSurfaceOpening(cave, state.Ground);
                CaveRouteMouthSnapUtility.FinalizeUndergroundEnvelope(cave, state.Ground);
            ...
            CaveBuildHealOrchestrator.CaptureIfEnabled(
                state.LadderReport?.BuildAcceptable == true ? "terrain_pipeline_complete" : "terrain_pipeline",
                state.Request?.Seed ?? 0,
                state.Ground?.Terrain,
                cave,
                forceAcceptable: state.LadderReport?.BuildAcceptable == true);
            CaveBuildEditorLog.LogSurface(
            ...
            out string message)
        {
            ...
            switch (phase)
            {
                ...
                case 4:
                    message = "DEM stamp must use queued path (RunDemStampPhase), not sync RunPhase.";
                    return false;
                case 5:
                    message = "NavMesh must use QueueBakePhase, not sync RunPhase.";
                    return false;
                default:
                    return false;
            }
