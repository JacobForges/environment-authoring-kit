...
namespace EnvironmentAuthoringKit.Editor.Blockout
{
    ...
    public static class CaveRouteProbeRunner
    {
        ...
        const int PathProbeBatchCells = 8;

        /// <summary>Route probe with per-batch queue activity (use from paced validation).</summary>
        public static ProbeReport RunPaced(Transform caveRoot) => RunInternal(caveRoot, pacedPathCells: true);

        public static ProbeReport Run(Transform caveRoot) => RunInternal(caveRoot, pacedPathCells: false);

        static ProbeReport RunInternal(Transform caveRoot, bool pacedPathCells)
        {
            ...
            for (var i = 0; i < layout.SolutionPath.Count;)
            {
                var batchEnd = pacedPathCells
                    ? Mathf.Min(i + PathProbeBatchCells, layout.SolutionPath.Count)
                    : layout.SolutionPath.Count;
                for (; i < batchEnd; i++)
                    ProbeRouteCell(caveRoot, layout, report, minClear, i);

                if (pacedPathCells && batchEnd < layout.SolutionPath.Count)
                    CaveBuildActionPacing.TouchQueueActivity();
            }
        ...
        static CaveBuildMetadata EnsureMetadata(Transform caveRoot)
        {
            ...
            var inferredSeed = CaveBuildWorkflowCoordinator.TryReadLastSeed(out var seed)
                ? seed
                : UnityObjectCompat.ReferenceId(caveRoot);
