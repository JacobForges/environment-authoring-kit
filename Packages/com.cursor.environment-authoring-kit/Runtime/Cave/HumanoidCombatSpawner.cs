...
namespace EnvironmentAuthoringKit.Cave
{
    ...
    public static class HumanoidCombatSpawner
    {
        ...
        /// <summary>Adds combat components to an existing humanoid (boss prefab, CC0 import, etc.).</summary>
        public static void EnsureCombatReady(
            GameObject go,
            int seed,
            CaveMobAggression aggression = CaveMobAggression.Aggressive)
        {
            if (go == null)
                return;

            HumanoidDimensions.Resolve(out var height, out var radius, out var stepOffset);
            EnsureCombatComponents(go, height, radius, stepOffset);
            ConfigureAggression(go, aggression);
            ConfigureEnemyBrain(go, seed);
        }
            ...
            Color? capsuleTint = null)
        {
            ...
            Cc0RuntimePrefabResolver.PrepareSpawnedInstance(instance);
            return instance;
        }
