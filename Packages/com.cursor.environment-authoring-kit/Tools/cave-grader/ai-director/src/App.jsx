...
const TABS = ["Preview", "Segments", "Pacing", "Voice", "Script", "Cards", "Export"];
...
export default function App() {
  ...
  const phase = session?.phase || "qna";
  const inBriefing = session?.inBriefing ?? (phase === "qna" || phase === "briefing_ready");

  const refresh = useCallback(async () => {
    if (!capture) {
      const latest = await api("/api/capture/latest");
      if (latest.capture) setCapture(latest.capture);
      return;
    }
