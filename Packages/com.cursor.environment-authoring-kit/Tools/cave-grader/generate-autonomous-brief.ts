...
function topFailing(report: QualityReport, n = 5): string[] {
  const thresholds = resolveCaveThresholds(report);
...
function main() {
  ...
  const thresholds = report ? resolveCaveThresholds(report) : null;
  const ship = (report?.overallScore ?? 0) >= (thresholds?.targetScore ?? 95);
  ...
  const nextLines = [
    ...
    "",
    `**Iteration:** ${iteration} | **Phase:** ${phase?.title ?? activeRung} | **Ship target:** ${thresholds?.targetScore ?? 95}+`,
    "",
