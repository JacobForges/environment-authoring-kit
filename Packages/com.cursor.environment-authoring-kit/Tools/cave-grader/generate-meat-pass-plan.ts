...
const WEB_QUERIES: Record<string, string[]> = {
  ...
  surface_navmesh: ["Unity NavMesh terrain trails walkable bake"],
  playable_world_gate: [
...
function main() {
  try {
    ...
    console.error(`[meat-pass-plan] non-fatal: ${msg}`);
    process.exit(0);
  }
}
...
function runMain() {
  ...
  const md = [
    ...
    (() => {
      try {
        return formatResearchExecutionBlock(hubRoot, "ground_placement", pass, "terrain");
      } catch {
        return "_Research brief unavailable this pass — use ResearchCache index + mountain hillshade refs._";
      }
    })(),
    "",
  ...
  const doNot = [
    ...
    "- Do NOT use Environment/Grid/EnvironmentRooms as FullWorld layout — see ResearchCache fullworld_do_not.",
  ].join("\n");
