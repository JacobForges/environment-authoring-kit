...
import { resolveCaveThresholds, stageFails } from "./grading-thresholds.js";
...
function caveThresholds(report: QualityReport) {
  return resolveCaveThresholds(report);
}
...
export function isRungFailing(
  ...
  const { stagePassScore, stageFloorScore } = caveThresholds(report);
  ...
  switch (rung) {
    ...
    case "visual_shell": {
      ...
      if (stageScore(report, "visual_shell") < stagePassScore) return true;
      return anyStageBelow(report, VISUAL_STAGE_IDS, stagePassScore);
    }
    case "ground_placement": {
      ...
      return anyStageBelow(report, GROUND_STAGE_IDS, stagePassScore);
    }
    case "floor_collision": {
      if (stageScore(report, "player_floor") < stagePassScore) return true;
      if (dud.some((r) => /fall|walkable|floor collider|fall-through/i.test(r)))
        return true;
      return anyStageBelow(report, FLOOR_COLLISION_STAGE_IDS, stagePassScore);
    }
    case "navmesh":
      return stageScore(report, "navmesh") < stagePassScore;
    case "materials":
      return (
        stageScore(report, "materials") < stagePassScore ||
        stageScore(report, "lighting") < stagePassScore
      );
    case "performance":
      return stageScore(report, "performance") < stageFloorScore;
    case "other":
...
function compactStageContext(report: QualityReport, rung: PromptRung): StageRow[] {
  ...
  const failing = report.stages.filter((s) => stageFails(s, caveThresholds(report)));
