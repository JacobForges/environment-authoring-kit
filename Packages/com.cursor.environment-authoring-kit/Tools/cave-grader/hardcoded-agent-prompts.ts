...
export const HARDCODED_PROMPTS_VERSION = "2026-06-04";
...
export const FULLWORLD_AAA_LAYOUT_RULES_MD = `## FullWorld / Full AAA (2026-06)

- **Full AAA Rebuild** places ~**289** terrain tiles (Chebyshev radius 8, 17×17) after the 9-tile play disk — then **LiDAR-guided sculpt** on every tile (not DEM stamp replacement).
- **49-tile** mountain rings (foothill + peak + labyrinth) still apply inside the larger grid; do not re-sculpt the locked 9 play tiles after directional sculpt completes.
- **Hollow Titan** — one **massive** hollow dead tree on a **random surface terrain tile** every seed (always above ground, never in cave). Boss portal proxy only.
- **No** radial star/checkerboard cave openings on play disk; **no** \`PlaceNineTileCaveOpenings\` star pattern.
- Labyrinth: south **two rows** of the 3×3 play disk (foothill maze trails); trails to **three** south peak cave mouths (west, center, east).
- Props/scatter on **all** kit Ground terrains for AAA; heavier rocks on peaks + foothills.`;
