...
6. **Additive surface** — Preserve Ground anchor and center land disk. LiDAR/DEM is **creative guide only** (procedural FBM + carve/rise sculpt + ≤38% structural bias on Full AAA), **never** a full heightmap stamp. Directional terraform sculpts every placed tile (9 play + rings; Full AAA ~289 tiles on 17×17).

6b. **Full AAA layout** — Flat grid after 9 play tiles → one massive **Hollow Titan** dead tree (random surface tile each seed) → biomes → weld → LiDAR-guided sculpt all tiles. No radial star cave openings; labyrinth on south **two rows** of play disk only; three south peak mouths (W/C/E).
