...
export function buildConsolidatedResearchPromptMd(
  ...
  const lines: string[] = [
    ...
    LAND_MASS_REFERENCE_RULES_MD,
    "",
    formatResearchCacheBlockForCategories(["fullworld_do_not"], hub, 8),
    "",
  ...
  const mountainPhase = phaseId
    ? PIPELINE_PHASES.find((p) => p.id === phaseId && p.id.startsWith("mountain_"))
    : undefined;
  if (mountainPhase?.researchCategories?.length) {
    lines.push(
      formatResearchCacheBlockForCategories(
        mountainPhase.researchCategories as ResearchCategory[],
        hub,
        12
      ),
      ""
