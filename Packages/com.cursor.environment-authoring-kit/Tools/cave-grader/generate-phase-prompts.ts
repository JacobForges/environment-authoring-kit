...
import { formatPhaseConceptPlanningBlock } from "./phase-concept-images.js";
...
function buildPhaseMarkdown(phaseId: string, iteration: number): string {
  ...
  const conceptBlock = formatPhaseConceptPlanningBlock(phaseId, hubRoot);
  if (conceptBlock) {
    lines.push(conceptBlock);
  }

  if (phase.allowed?.length) {
    lines.push("## Allowed (this phase only)", "");
    for (const a of phase.allowed) lines.push(`- ${a}`);
    lines.push("");
  }

  if (phase.forbidden?.length) {
    lines.push("## Forbidden (this phase only)", "");
    for (const f of phase.forbidden) lines.push(`- ${f}`);
    lines.push("");
  }

  if (phase.researchCategories?.length) {
    lines.push(
      formatResearchCacheBlockForCategories(
        phase.researchCategories as ResearchCategory[],
        hubRoot,
        10
      ),
      ""
