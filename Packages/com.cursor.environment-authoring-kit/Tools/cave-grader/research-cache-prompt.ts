...
/** Prompt block scoped to explicit research categories (mountain phases). */
export function formatResearchCacheBlockForCategories(
  categories: ResearchCategory[],
  hubRoot: string,
  limit = 10
): string {
  const lookup = lookupForCategories(hubRoot, categories, limit);
  if (!lookup) {
    return [
      "## Research cache (mountain categories)",
      "",
      "Run `npm run sync-research-cache` in Tools/cave-grader.",
      "",
    ].join("\n");
  }
  ...
  const { index, hits } = lookup;
  const lines: string[] = [
    "## Research cache — category-scoped (read local FIRST)",
    "",
    `**Categories:** ${categories.join(", ")}`,
    `**Index:** \`${hubRoot}/${RESEARCH_CACHE_INDEX_REL}\` (${index.stats.totalEntries} entries)`,
    "",
    "### Entries for this phase",
  ];

  for (const e of hits) {
    lines.push(`- **${e.title}** (\`${e.category}\`, ${e.year})`);
    lines.push(`  - \`${hubRoot}/${e.contentPath}\``);
    lines.push(`  - ${e.serialized.summary.slice(0, 180)}`);
  }
  ...
  for (const cat of categories) {
    ...
    if (!block?.entryIds.length) continue;
    lines.push(
      `- Browse \`${cat}\`: \`${hubRoot}/Assets/EnvironmentKit/ResearchCache/categories/${cat}/index.json\` (${block.entryIds.length})`
    );
  }
