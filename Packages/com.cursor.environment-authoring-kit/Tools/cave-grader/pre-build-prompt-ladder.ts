...
export function parseWorkflowArg(
  ...
  if (env === "pre_build") return "pre_build";
  if (env === "post_build") return "post_build";
  if (env === "gameplay") return "gameplay";
  if (env === "terrain" || env === "terrain_geo" || env === "terrain_props") {
    return env as "terrain" | "terrain_geo" | "terrain_props";
  }
  for (const arg of argv) {
    if (arg.startsWith("--workflow=")) {
      ...
      if (v === "gameplay") return "gameplay";
      if (v === "terrain" || v === "terrain_geo" || v === "terrain_props") {
        return v as "terrain" | "terrain_geo" | "terrain_props";
      }
    }
  }
  return null;
