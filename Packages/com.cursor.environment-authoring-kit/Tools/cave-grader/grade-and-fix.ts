...
import { appendHealLedgerEvent } from "./heal-checkpoint.js";
import { preSessionPromptAugment } from "./bot-supervisor.js";
import {
  buildGameplayLadderPrompt,
  ensureHubGameProgressFile,
  parseGameplayMilestoneArg,
  pickActiveGameplayMilestone,
  type GameplayMilestoneId,
} from "./gameplay-prompt-ladder.js";
import {
...
const isGameplay = workflowMode === "gameplay";
const isTerrain =
  workflowMode === "terrain" ||
  workflowMode === "terrain_geo" ||
  workflowMode === "terrain_props";
const terrainWorkflow: TerrainWorkflowMode | null = isTerrain
  ? parseTerrainWorkflowArg(process.argv) ??
    (workflowMode === "terrain_geo"
      ? "terrain_geo"
      : workflowMode === "terrain_props"
        ? "terrain_props"
...
function providerNeedsApiKey(name: string): boolean {
  const p = name.toLowerCase();
  return p !== "localollama" && p !== "locallmstudio";
}
      ...
      eventType:
        exitCode === 0
          ...
          : exitCode === undefined
            ...
            : "ai_invoke_failed",
      workflow,
      note: String(payload.error ?? payload.status ?? payload.skipped ?? "grade-and-fix"),
      rung: typeof payload.rung === "string" ? payload.rung : undefined,
      exitCode: typeof exitCode === "number" ? exitCode : undefined,
      runId: typeof payload.runId === "string" ? payload.runId : undefined,
      aiProvider: typeof payload.provider === "string" ? payload.provider : undefined,
    });
  } catch {
    /* ignore */
  }
...
function isPathAllowed(relPath: string): boolean {
  ...
  return (
    ...
    p.startsWith("Assets/Scripts/") ||
    p.startsWith("Assets/Narrative/") ||
    p.startsWith("Assets/Tests/") ||
    p === "README.md" ||
    p.startsWith("docs/")
  );
}
...
async function main(): Promise<void> {
  ...
  if (!apiKey && providerNeedsApiKey(provider)) {
    const keyHint = isCursorProvider
      ...
      : `API key required for provider '${provider}' (CAVE_ACTIVE_API_KEY or provider-specific key in .env; set in Hub → Settings). Local Ollama / LM Studio do not need a cloud key.`;
  ...
  if (isTerrain) {
    ...
    activeRung = pickActiveTerrainRung(terrainReport, overrideRung, terrainWorkflow);
    ...
    if (useExportedPrompt && existsSync(tailoredOnDisk)) {
      ...
      prompt = buildTerrainLadderPrompt({
        ...
        workflow: terrainWorkflow,
      });
    }

    const terrainThresholds = resolveTerrainThresholds(terrainReport);
    const failing = terrainReport.stages
      .filter((s) => stageFails(s, terrainThresholds))
      .map((s) => `${s.id}:${s.score}`);
    console.log(
      `[CaveCursor:info] Terrain workflow=${terrainWorkflow ?? "terrain"} rung=${activeRung} grade=${terrainReport.letterGrade} (${terrainReport.overallScore}/100)`
    );
    console.log(`[CaveCursor:info] Failing terrain: ${failing.join(", ") || "none"}`);

    if (!isTerrainRungFailing(activeRung, terrainReport, terrainWorkflow)) {
      emitPhaseComplete(terrainWorkflow ?? "terrain", activeRung, "already_passing");
    ...
    if (!worldGatePassed(hubRoot)) {
      ...
      writeLastRunDiagnostics(workflowMode, {
        exitCode: 1,
        error: "world_gate_not_passed",
      });
      process.exit(1);
    }

    const progressPath = ensureHubGameProgressFile(hubRoot);
    const progress = loadHubGameProgress(hubRoot);
    const overrideMilestone = parseGameplayMilestoneArg(process.argv);
    activeRung = pickActiveGameplayMilestone(progress, overrideMilestone);
    ...
    prompt = buildGameplayLadderPrompt({
      hubRoot,
      activeMilestone: activeRung as GameplayMilestoneId,
      progressPath,
    });
    tailoredExportPath = join(generated, "HubGameplayActivePrompt.md");

    console.log(formatMissionPhaseLine(hubRoot));
    console.log(
      `[CaveCursor:info] Gameplay workflow milestone=${activeRung} demoReady=${progress?.demoReady === true}`
    );
  } else {
    ...
    const caveThresholds = resolveCaveThresholds(report);
    const failing = report.stages
      .filter((s) => caveStageFails(s, caveThresholds))
      .map((s) => `${s.id}:${s.score}`);
    console.log(
  ...
  prompt = isGameplay
    ? prompt
    : prependAutonomousPromptBlocks(prompt, activeRung, isTerrain);
  if (process.env.CAVE_BOT_SUPERVISOR === "1") {
    const augment = preSessionPromptAugment(hubRoot);
    if (augment) prompt = `${augment}\n\n---\n\n${prompt}`;
  }
  if (!isCursorProvider) {
    prompt +=
  ...
  if (isTerrain) {
    ...
    writeFileSync(join(generated, "HubGameplayActivePrompt.md"), prompt, "utf8");
  } else {
  ...
  async function completeSessionAfterAgent(
    result: RunResult,
    extraDiagnostics: Record<string, unknown> = {}
  ): Promise<void> {
    await sleep();
    console.log("[CaveCursor:info] Verifying compile before marking session done…");
    const verify = verifySessionAfterAgent(hubRoot);
    for (const note of verify.notes) console.log(`[CaveCursor:verify] ${note}`);

    const wf: WorkflowMode = isPreBuild
      ? "pre_build"
      : isTerrain
        ? (terrainWorkflow ?? "terrain")
        : "post_build";
    ...
    if (!verify.ok) {
      ...
      writeLastRunDiagnostics(workflowMode, {
        exitCode: 4,
        error: "session_verify_failed",
        reason: verify.reason,
        compileErrorCount: verify.compileErrors.length,
        rung: activeRung,
        runId: result.id,
    ...
    console.log("[CaveCursor:info] Verify OK — emitting phase-complete flag");
  ...
  try {
    ...
    await completeSessionAfterAgent(result);
  } catch (err) {
    if (isSqliteStartupFailure(err) && repoUrl && useAuto && !forceCloud) {
      ...
      try {
        ...
        if (cloudResult.result) console.log(cloudResult.result);
        await completeSessionAfterAgent(cloudResult, { runtime: "cloud_after_sqlite" });
        process.exit(0);
      } catch (cloudErr) {
