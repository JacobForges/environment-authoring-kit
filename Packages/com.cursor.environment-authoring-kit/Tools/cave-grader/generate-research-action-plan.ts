...
import {
  FULLWORLD_DO_NOT_PROMPT_BULLETS,
  isFullWorldSurfacePhase,
} from "./fullworld-do-not-prompt.js";
import { FULLWORLD_LAYOUT_IDEAL_PROMPT_BULLETS } from "./fullworld-layout-ideal-papers.js";
...
function researchUrlsForPhase(phaseId: string, limit = 8): { id: string; url: string; localMd: string }[] {
  ...
  if (phase?.researchCategories?.length) {
    const cats = phase.researchCategories as ResearchCategory[];
    const lookup = lookupForCategories(hubRoot, cats, limit);
    for (const e of lookup?.hits ?? []) {
      const url = (e as CacheEntry & { url?: string }).url ?? (e as CacheEntry & { sourceUrl?: string }).sourceUrl;
      if (!url) continue;
  ...
  for (const e of entries) {
    ...
    const match =
      !phase ||
      phase.researchCategories.some((c) => cat === c || cat.includes(c) || e.id?.includes(c.replace("_", "")));
  ...
  return {
    ...
    layoutRandomness:
      seed > 0
        ? {
            buildSeed: seed,
            rule: "Honor buildSeed — cave maze flavor, mountain labyrinth grid, and prop scatter must vary per run; read SurfaceWorldLayoutPrePlan.json.",
            prePlanJson: "Assets/EnvironmentKit/Generated/SurfaceWorldLayoutPrePlan.json",
          }
        : undefined,
    ...
    doNot: [
      ...(isFullWorldSurfacePhase(phaseId) ? FULLWORLD_DO_NOT_PROMPT_BULLETS : []),
      ...(isFullWorldSurfacePhase(phaseId) && isIdealLayoutStyle(hubRoot)
        ? FULLWORLD_LAYOUT_IDEAL_PROMPT_BULLETS
        : []),
