...
"""Start or refresh AI Director API + static UI.
  ...
  python3 ensure-ai-director.py [/path/to/project] [--open|--no-open]
"""
...
from director_paths import latest_project, server_runtime_dir, studio_repo_root  # noqa: E402

_planner.load_dotenv(None)
if not _planner.cursor_api_configured():
    if _planner.seed_cursor_api_key_from_cave_grader():
        print(f"Seeded CURSOR_API_KEY into {_planner.studio_env_path()}", flush=True)
    elif not _planner.cursor_api_configured():
        print(
            f"Warning: CURSOR_API_KEY not set — edit {_planner.studio_env_path()}",
            file=sys.stderr,
        )
...
def api_has_music_routes(port: int) -> bool:
    code, body = _http_get(f"http://127.0.0.1:{port}/api/director/health")
    if code != 200:
        return False
    try:
        ...
        return False
    return bool(payload.get("musicRoutes"))
...
def dist_needs_rebuild() -> bool:
    ...
    for rel in (
        ...
        "MediaIngestView.jsx",
        "BriefingView.jsx",
        "MusicBriefingView.jsx",
        "MusicSetupView.jsx",
        "MusicRecordView.jsx",
        "MusicProduceView.jsx",
        "MusicStepper.jsx",
        "index.css",
    ):
...
def start_server(project: Path | None, port: int) -> None:
    _LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
    log_fd = open(_LOG_FILE, "a", encoding="utf-8")
    env = _planner._planner_subprocess_env(studio_repo_root())
    cmd = [_python(), str(_SERVER), "--port", str(port)]
    if project:
        cmd.extend(["--capture", str(project)])
...
def register_gate(project: Path, port: int) -> None:
    enc = urllib.parse.quote(str(project.resolve()))
    _http_post(f"http://127.0.0.1:{port}/api/capture/compose/gate?path={enc}", {"waiting": True})
    if os.environ.get("STANDALONE", "1") == "1":
        _http_post(f"http://127.0.0.1:{port}/api/capture/compose/gate/verify?path={enc}", {"capture": str(project)})


def director_url(project: Path | None, port: int) -> str:
    if project is None:
...
def ensure(project: Path | None, *, open_ui: bool) -> int:
    port = _port()
    resolved = project.expanduser().resolve() if project else latest_project()

    ensure_dist_built()
    if not api_healthy(port) or not api_has_routes(port) or not api_has_music_routes(port):
        stop_api(port)
    ...
    if resolved is not None and resolved.is_dir():
        ...
        url = director_url(None, port)
    else:
        url = director_url(None, port)
    print(f"AI Director Studio: {url}", flush=True)
...
def main() -> int:
    ...
    return ensure(args.project, open_ui=open_ui)
