...
from director_paths import is_standalone, projects_dir, server_runtime_dir  # noqa: E402

COMPOSE_SERVER_LOG = server_runtime_dir() / "recap-dashboard-server.log"
...
def set_gate_proceed(capture: Path) -> dict:
    ...
    if not gate.get("assetsVerified"):
        if is_standalone():
            gate = set_gate_assets_verified(capture)
        else:
            raise ValueError(
...
def ensure_gate_ready_for_compose(capture: Path) -> dict:
    ...
    if gate.get("assetsVerified"):
        return gate
    if is_standalone() or capture_has_approved_cards(capture):
